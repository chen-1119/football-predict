...
组合比赛与结算记录直接来自 PostgreSQL。新增 009 迁移；事务和唯一约束保证每日每档最多冻结一次，冻结字段与结算字段分列保存。任何官方取消比赛使整组 VOID，单列计数，不进入命中率。重复比赛、共享球队和同联赛过度集中均阻断。

全局模型风险、推荐可靠性、采集新鲜度及发布身份全部通过后才生成候选。页面通过有访问权限校验的 `/api/v1/daily-featured-combos` 读取，分开显示待冻结与已冻结，读取失败不报零。定时器每 5 分钟运行，上限 60 秒和 256MB，无 JSON 或 SQLite 回写。规则收紧是保守策略，尚不代表新样本命中率已提高。

已完成规则测试、推荐边界回归、前端构建及真实 PostgreSQL 回滚事务测试。本地旧归档校验存在两项数量不一致，生产数据不随本次包传输，以线上 PostgreSQL 和冻结记录校验为准。
