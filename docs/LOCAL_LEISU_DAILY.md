...
7. 未匹配的逐场原因放在最后一批 `missing:[{siteMatchId,reason:'unmapped'}]`。来源阻断时仅为未能继续的比赛填写 blocked/login-required/browser-unavailable。已经开赛或官方状态冲突由服务端列为 ineligible。空清单或浏览器不可用也提交真实空 entries 状态，不拿其他日期填任务。
