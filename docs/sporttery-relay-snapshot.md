...
```bash
SPORTTERY_RELAY_PUSH_BASE_URL=https://170.106.75.73 \
...
```bash
curl -X POST "https://170.106.75.73/api/admin/sporttery-relay-snapshot?runSync=1" \
...
```bash
curl -X POST "https://170.106.75.73/api/admin/sporttery-relay-snapshot?validateOnly=1" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  --data-binary @<(jq '{snapshot: .}' /tmp/sporttery-relay-snapshot.json)
```
...
```bash
FOOTBALL_CLOUD_API_BASE=https://170.106.75.73 \
...
```bash
scp -P 18789 /tmp/sporttery-relay-snapshot.json ubuntu@170.106.75.73:/tmp/
ssh -p 18789 ubuntu@170.106.75.73 \
