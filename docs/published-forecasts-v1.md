...
## 客户端接管验证与上线修复（2026-09-17）

- 完整构建补出了 Hook 和发布台账解析器缺少结尾括号的问题，已修复；测试转译开启语法诊断。
- `/best` 与 `/betslip` 共用服务端发布/串关台账，不再依赖旧正式单场数量。
- 真实生产输入发现，旧不可变赛前决策会保留前一天的模型时间。新增 `prospectiveForecastInput`，只从本轮实际计算的同场、同球队、同开赛版本取完整模型与官方报价；不刷新旧模型时间、不修改旧冻结方向。无本轮数据时不生成新输入，停售/开赛后禁止发布。
- 串关概率按实际总和归一化，报价证据哈希纳入真实观察时间；单场及组合均在事务提交前检查停售截止。
- 客户端完整 React/TypeScript/Vite 构建通过。57 项发布/解析/输入回归、23 项组合断言、既有决策时钟链路验证通过。
- 已在服务器原生 PostgreSQL 的临时 schema 内完成发布、不可变触发器、幂等、结算更正和组合结算测试，全部 ROLLBACK；测试不写生产业务行。
- 以上补充取代上文网页端仅局部测试的验证范围。生产部署状态以对应发布回执为准，不由文档单独证明。模型准确性尚未通过新样本外验证。
