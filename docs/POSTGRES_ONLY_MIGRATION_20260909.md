...
## 2026-09-10 原生监控、候选增量库与真实备份恢复

- `checkServerRuntime` 从允许名单配置识别 postgres-only。原生监控直接核对 PG 主读、退役状态、完整 generation 与赛果回执；即使旧 unit 仍带 SQLite 自动修复选项，也不会执行导出、重建或第二套数据库写入。投影恢复归唯一 sync worker 管理，故障继续报告 failed，不将禁用修复当作健康。
- `verifyProductionPlanCoverage` 在原生模式使用同一个 PG 只读快照查询真实 current/history/odds/prediction 数量及回执，不再检查旧库 legacy JSONL 导入版本。SQLite 静态兼容测试仍保留，不能把兼容性测试当作生产依赖或跳过其它模型门槛。
- 监控真实子进程的正常、坏回执、SQLite 意外返回及直接修复拒绝四组测试通过；遗留运行时稳定性 15 组通过；原生健康证明 15 项通过。隔离 API 测试补齐 storage/backend 环境隔离，postgres-only 和 hybrid 六种检查组合均通过，不继承生产数据库连接。
- 原生归档读屏障改为立即尝试 shared advisory lock；遇到正在投影的写者明确返回可重试错误，不无限等待。连接层超时导致锁状态不确定时丢弃连接，避免迟到响应把 session lock 留在连接池。
- `postgresReleaseMirror` 提供独立候选数据库的全量初始化和增量刷新库。来源强制 REPEATABLE READ/read-only，目标验证实际 `football_release_*` 名称且不得等于来源库。校对全部表结构、视图、约束、生成列及主键；原始每列文本/bytea/JSON 与时钟逐行往返核对。对候选库的写入、删除与镜像凭据在单个事务中完成，源库不写入。
- 增量不是按业务时间戳猜测：源与目标 xmin 配合根权限持有的独立 HMAC 密钥认证行状态；候选模型计算造成的改动会触发原始行重拷。数据库身份/结构变更拒绝自动复用，事务标识跨越安全范围强制重新初始化。原始平局、同时间戳变更、生成计数、复合主键、二进制、并发源写入、伪造凭据及错误生产目标均有真实双库验证。
- 避免大表随机读取：元数据一次顺序扫描，每批 2048 行放入候选临时表；不按主键随机遍历大复盘堆，也不重复扫描来源。大载荷仍限每批 128 行；暖候选无变化时复制载荷字节为 0。该模块尚未接入完整签名发布/恢复入口，不能据此宣称生产发布已提速。
- 最新合并 QA 集群：57 个原生场景、467 项真实数据库/API 检查通过，原生运行时 SQLite 访问为 0。报告 `outputs/q1-native-postgres-evidence-result.json`。ESLint、现有发布约束及原始冻结方向检查保持。
- 10:30–10:34 北京时间，真实最新备份（670,772,639 字节，SHA-256 校验通过）在单独新建且禁止 PUBLIC 连接的 QA 数据库完整恢复；用时 217,767 ms。恢复得到 50,000 条预测、1,323 条冻结推荐、1,304,369 条复盘和原始快赛果回执。库大小 5,027,675,159 字节；按唯一归属标记核验后已删除本次 QA 库，未重启服务、未写生产业务库。证据 `outputs/postgres-backup-restore-rehearsal-1789007672980.json`。这证明备份可恢复，不等于已实现应用回滚或迁移切换。
...
## 2026-09-10 11:51 发布风险收尾与资源限制

- 上述 v2 交接及“压测进行中”已过时：最后一轮在 11:42:58 北京时间失败结束。恢复种子用时 189,429 ms；随后核对 1,648,453 行（复制 8,210 行、核对种子 1,640,243 行），在提交前超过 600 秒预算，事务未提交。独立库 `football_release_69f5dc87db12_1789010977` 已清理，未写生产业务库、未重启服务。报告 `outputs/native-candidate-mirror-benchmark-1789011778145.json`。不再启动同类全库压测，不能把扫描改用 PostgreSQL 说成已解决发布耗时。
- 修复旧发布入口缺少存储分流保护：经过认证的完整包在消费发布序号前，读取固定环境文件并检查存储模式。原生/部分迁移配置或重复、含糊的选择器立即拒绝，禁止进入会强制 `ENABLE_SQLITE_EXPORT=1` 的 v3 路径。入口脚本也在首次加载 SQLite 及任何主机变更前执行同一检查，兼容旧的已安装包装器（旧包装器仍可能已消费序号）。纯前端入口不受影响。这是防错闸门，不是原生发布实现。
- 新增 v4 原生恢复读取支持：记录本机 PostgreSQL 集群标识、数据库 OID 及绑定的旧应用 SHA；根据真实数据库名字/OID 判断是否已切换，不仅相信阶段文件。切换后的应用回滚保留最新数据库和模型账本，不恢复 SQLite/旧模型快照，不把存储模式改回 hybrid。数据库身份混乱、环境内容不匹配、健康接口只有 HTTP 200 而缺少原生数据证明时保留事务，拒绝报成功。原 v3 恢复保持兼容。
- 本轮仅做对应小测试：存储入口 20 项和原生恢复 20 场景通过；旧恢复 284 断言、14 回滚阶段/3 提交阶段及合成 SQLite/WAL 镜像恢复通过。数据库身份/systemd 的原生恢复测试为模拟，不代表真实生产切换演练完成。新增发布脚本通过语法及 ESLint 检查。
- 发布专用的新增检查脚本列入精确的模型复用白名单，避免这些文件的变化单独触发模型重算；真正模型/采集/未知脚本变化仍要求重算，模型证据门槛不降低。

**仍未上线、未完成 SQLite 退役。** 真正剩余的是原生发布执行入口和一次性受保护的数据切换；v4 恢复读取器还没有被生产发布器生成的事务调用。不得修改线上选择器后仍调用旧发布流程，不得将已合并代码或本地回归当作部署证明。最后线上只读证据仍为 11:29:27 的 r718/r719；本节没有重新轮询生产。

## 2026-09-11 发布验收错配修复与兼容发布准备

- 9 月 10 日 r723 的正常签名发布失败，耗时 22 分 23 秒，未切换应用。具体失败不是 PostgreSQL 复制，而是隔离测试已经执行 3 个脚本 × 两种存储配置共 6 个成功场景，生产验收仍硬编码要求 3 个。旧服务恢复后继续服务，不能将该包标记为已接受的原生兼容基线，也不重发已消费的序号。
- 新的 `productionFixtureIsolationContract` 由测试生产者和验收入口共享，逐一核对脚本/存储模式、退出状态、信号和错误，拒绝遗漏、重复、未知和失败场景。上传前的发布约束测试执行真实生产者源码（子进程替身）与真实消费者判定，共 23 项；这样同类错配在构建签名前失败，不再等候选服务启动才发现。六个真实隔离场景于 10:26:42 北京时间通过，未访问生产数据库。
- 原生 HTTP 启动改为只读检查已安装的全部迁移版本及 SHA；DDL 留给显式迁移，hybrid 保留现有迁移方式。独立 PostgreSQL 16.15 的只读角色测试已通过 8 项（含实际拒绝 DDL/数据修改），测试库清理完成；本次不重跑未改动的大库测试。
- v4 日志写入器接入发布事务初始化钩子，写入原生环境及集群/OID 合约，不执行数据库改名或激活。写入器到实际恢复 CLI 的 8 项隔离检查通过（systemd/数据库身份模拟）。首次迁移基线保持空值并拒绝初始切换，因为 r723 未成功；普通发布仍走 v3，不能宣称原生发布已完成。
- 10:23:47 北京时间线上只读核验：后端 r718 / 前端 r719，服务与 worker active，recoveryPending=false，数据新鲜、赛果完整性和推荐投影一致性正常，采集 2/2。模型风险门槛尚未通过，recommendationReliable=false，不变更原始推荐、统计或门槛来掩盖该状态。

本节为修复和新包准备记录，不是部署成功证明。后续按新的签名序号部署兼容版本，再单独完成受保护的原生发布切换；当前不删除仍被生产依赖的 SQLite 或回退资料。

## 2026-09-11 11:02 r724 失败后的实际内存修复

- r724（SHA `f5970c9b495470ef846da6f21ea7b94054926058d87d75170e56b37922409d11`）本地构建 259,195 ms，包 25,431,584 字节，不含数据库。正常部署于 10:34:59 开始、10:52:08 失败，耗时 17 分 9 秒，没有切换应用。不得重发该序号或将其写成已接受的原生基线。
- 本轮实测模型回测 28 秒、策略处理 17 秒；主要停滞出现在候选 SQLite 导出。进程在 `mem_cgroup_handle_over_high` 状态，约 2.3 GB cgroup 内存、512 MiB 交换区，数分钟读写计数不变。软限流线 2100 MiB，硬上限 2600 MiB。第一轮临时调整因主机余量不足在写入前拒绝；余量恢复并重新满足 2 GiB 预留后，只将该唯一临时任务的 MemoryHigh 调为 2500 MiB，硬上限、交换区、10 分钟超时及所有验收门槛均未改。约 7 秒后任务成功结束。临时 unit 已回收，LoadState=not-found、DropInPaths 为空，没有持久配置遗留。
- 下一阶段截止证据任务因 V8 内存溢出退出 134，尚未运行生产验收。根因证据：真实快照文件 530,469,288 字节，observations 超过 8 MiB 前缀上限，旧逻辑转为全文件 JSON.parse。前缀检查确实返回 `array-exceeds-prefix-limit`，错误栈为 JSON 解析 OOM；这不是前一版测试数量错配再次发生。
- 新增 `readCandidatePublicObservations`：前缀不足时分块计算稳定文件摘要，用现有严格流式 JSON 读取器检查完整文件，但只物化 observations。不丢弃观测、历史不截断、不重写预测；文件并发变化、非法 JSON、字段类型或内存准入异常均拒绝。兼容的小文件路径保持。
- 新增上传前受限内存回归：111 MB 合成文件、超过 8 MiB 的观测数组，在 96 MiB V8 堆上限下执行真实调用分支；8 项检查通过，峰值 125,784 KiB，完整原始平局及最后一条观测保留。原截止证据回归通过。随后在 384 MiB 堆上限下只读真实 immutable generation，530 MB 文件 78 条观测全部读取，17,313 ms，峰值 127,688 KiB，业务写入 0。证据 `outputs/live-snapshot-stream-readonly-1789095657756.json`。不是整库压测或部署验收。
- SQLite 导出已有释放阶段载荷并调用 global.gc 的代码，但 npm 入口此前没有暴露 GC。`datastore:sqlite` 现启用 `--expose-gc`，投影命令及数据语义不变；同样启用该参数的增量导出回归 198 项通过，上传前检查确保实际命令有效。尚不将此宣称为生产峰值已降低或 SQLite 已退役。
- 10:52:57 最新恢复检查：仍 r718/r719，service/worker active，recoveryPending=false，数据与赛果完整性正常，采集 2/2。本轮未删除本地包/数据库；4 个失败包约 101 MB 仅完成清单核验，源码、备份、当前包及回退资料保留。

接下来先将本节内存修复提交并合并，新的构建必须通过上传前容量检查；继续原生发布接线和切换目标，但不能使用失败的 r724 作为兼容回滚基线。
