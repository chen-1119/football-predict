...
readonly TLS_IP_ADDRESS="170.106.75.73"
readonly TLS_CERT_NAME="170.106.75.73"
readonly HTTPS_PUBLIC_BASE_URL="https://170.106.75.73"
readonly HTTP_PUBLIC_BASE_URL="http://170.106.75.73"
readonly PUBLIC_BASE_URL_FILE="/etc/football-release/public-base-url"
readonly RENEWAL_CONFIG="/etc/letsencrypt/renewal/170.106.75.73.conf"
...
promote_ready_final_tls_after_interruption() {
  [ -L "$TLS_SITE_ENABLED" ] \
    ...
    && grep -F 'return 308 https://170.106.75.73$request_uri;' "$TLS_SITE_AVAILABLE" >/dev/null \
    && grep -F 'ssl_certificate /etc/letsencrypt/live/170.106.75.73/fullchain.pem;' "$TLS_SITE_AVAILABLE" >/dev/null \
    && nginx -t \
...
for command_name in bash certbot cp curl dirname env find grep head install ln mktemp mv nginx readlink rm sed stat sync systemctl timeout; do
