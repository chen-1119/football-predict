...
(async () => {
  ...
  const checks = [], errors = [];
  try {
    for (const width of [320, 390, 768, 1440]) for (const lang of ['zh', 'en']) for (const mode of ['complete', 'missing']) {
    ...
    const report = { ok: true, checkedAt: new Date().toISOString(), scope: 'actual DataAdoptionDetails TSX and CSS, synthetic consumer fixtures only', checks, runtimeErrors: errors };
