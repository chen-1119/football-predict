...
verify("valid 500.com HAD source produces a home reference", () => {
  const result = build();
  assert.ok(result);
  assert.equal(result.version, "external-odds-analysis-reference-v4");
...
verify("a leader just below fifty-five percent is rejected even with a wide gap", () => {
  assert.equal(build({
    odds: { odds1: 1 / 0.5499, oddsX: 1 / 0.3001, odds2: 1 / 0.15 },
  }), null);
});

verify("exactly fifty-five percent is accepted", () => {
  const result = build({ odds: { odds1: 1 / 0.55, oddsX: 1 / 0.30, odds2: 1 / 0.15 } });
  assert.ok(result);
...
verify("an eight-point gap alone is insufficient below the absolute probability floor", () => {
  assert.equal(build({
    odds: { odds1: 25 / 11, oddsX: 25 / 9, odds2: 5 },
  }), null);
