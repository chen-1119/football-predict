...
const replayExpectationForSnapshot = (snapshot) => {
  ...
  if (policyVersion === LEGACY_MULTI_FACTOR_POLICY_VERSION) return "legacy-multi-factor-audit-only";
...
let currentLegacyMultiFactorV1AuditOnlyRows = 0;
let currentLegacyMultiFactorV1AuditOnlyCandidates = 0;
let currentMultiFactorV2ExactCandidates = 0;
  ...
  for (const row of currentDecisionRows) {
    ...
    if (snapshot.version === DECISION_SNAPSHOT_VERSION) {
      ...
      if (replayExpectation === "legacy-multi-factor-audit-only") {
        currentLegacyMultiFactorV1AuditOnlyRows += 1;
      }
    }
    ...
    for (const candidate of candidates) {
      ...
      if (snapshot.version === DECISION_SNAPSHOT_VERSION) {
        ...
        if (replayExpectation === "current-policy-exact") {
          currentMultiFactorV2ExactCandidates += 1;
          assert.equal(
            replay.exact,
            true,
            "every current multi-factor v2 candidate must replay exactly",
          );
        } else {
          currentLegacyMultiFactorV1AuditOnlyCandidates += 1;
          assert.equal(candidate.evidenceVersion, LEGACY_MULTI_FACTOR_POLICY_VERSION);
          assert.equal(candidate.evidenceReplay?.canonicalOutput?.version, LEGACY_MULTI_FACTOR_POLICY_VERSION);
...
console.log(JSON.stringify({
  ...
  currentLegacyMultiFactorV1AuditOnlyRows,
  currentLegacyMultiFactorV1AuditOnlyCandidates,
  currentMultiFactorV2ExactCandidates,
