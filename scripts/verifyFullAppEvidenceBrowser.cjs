...
(async () => {
  ...
  try {
    for (const width of [390, 768, 1440]) {
      ...
      await page.goto(`${baseUrl}/review`);
      await page.getByRole('button', { name: /数据参考/ }).click();
      await page.locator('[data-review-paired-rate]').waitFor();
      assert.equal(await page.locator('[data-review-overview-rate]').textContent(), '50.0%');
      assert.equal(await page.locator('[data-review-paired-rate]').textContent(), '0.0%');
      assert.match(await page.locator('[data-review-paired-reference]').textContent(), /100.0%/);
      assert.match(await page.locator('[data-review-paired-count]').textContent(), /可配对 1 \/ 本窗口已结算 2 · 排除 1/);
      await page.locator('[data-review-paired-baseline] summary').focus(); await page.keyboard.press('Enter');
      assert.equal(await page.locator('[data-review-paired-baseline] details').getAttribute('open'), '');
      assert.ok((await page.locator('[data-review-paired-baseline]').textContent()).includes('不能直接与全量命中率比较'));
      await page.evaluate(() => window.scrollTo(0, 0)); await audit('paired-subset-not-full-ledger-keyboard-open');
      await page.screenshot({ path: path.join(outputDir, `full-app-review-pairs-${width}.png`), fullPage: true });
      await page.getByRole('button', { name: '近 7 天', exact: true }).click();
      assert.equal(await page.locator('[data-review-paired-rate]').textContent(), '—');
      assert.match(await page.locator('[data-review-paired-count]').textContent(), /可配对 0 \/ 本窗口已结算 0 · 排除 0/);
      await page.evaluate(() => window.scrollTo(0, 0)); await audit('paired-empty-window-no-fabricated-percent');
      await page.getByRole('button', { name: '近 30 天', exact: true }).click();
      assert.equal(await page.locator('[data-review-paired-rate]').textContent(), '0.0%');
      await page.getByRole('button', { name: '按版本', exact: true }).click();
      assert.equal(await page.locator('[data-review-paired-rate]').count(), 0);
      await page.locator('[data-review-version-picker] select').selectOption(fixture.pairSummary.versionBreakdown.groups[0].key);
      await page.getByRole('button', { name: 'HHAD 让球', exact: true }).click();
      assert.match(await page.locator('[data-review-paired-count]').textContent(), /可配对 1 \/ 本窗口已结算 1 · 排除 0/);
      await page.evaluate(() => window.scrollTo(0, 0)); await audit('paired-explicit-version-independent-handicap');
      await page.getByRole('button', { name: /正式推荐/ }).click();
      assert.equal(await page.locator('[data-review-paired-rate]').count(), 0);
      await page.getByRole('button', { name: /研究影子/ }).click();
      assert.equal(await page.locator('[data-review-paired-rate]').count(), 0);
      await page.evaluate(() => window.scrollTo(0, 0)); await audit('formal-shadow-do-not-borrow-reference-pairs');
      mode = 'pairs-invalid';
      await page.goto(`${baseUrl}/review`);
      await page.getByRole('button', { name: /数据参考/ }).click();
      await page.waitForFunction(() => document.querySelector('[data-review-overview-rate]')?.textContent === '50.0%');
      assert.equal(await page.locator('[data-review-paired-rate]').count(), 0);
      assert.ok((await page.locator('[data-review-paired-baseline]').textContent()).includes('待补证'));
      await audit('invalid-pair-ledger-preserves-main-hit-rate');
      mode = 'versions';
      await page.goto(`${baseUrl}/review`);
      await page.locator('[data-review-overview]').waitFor();
      await page.getByRole('button', { name: /数据参考/ }).click();
      await page.getByRole('button', { name: '按版本', exact: true }).click();
      await page.locator('[data-review-version-picker] select').waitFor();
      assert.equal(await page.locator('[data-review-overview-rate]').textContent(), '—');
      const key = fixture.versionSummary.versionBreakdown.groups.find(g => g.modelVersion === 'frozen-model-b').key;
      await page.locator('[data-review-version-picker] select').selectOption(key);
      assert.equal(await page.locator('[data-review-overview-rate]').textContent(), '—');
      await page.getByRole('button', { name: 'HHAD 让球', exact: true }).click();
      assert.equal(await page.locator('[data-review-overview-rate]').textContent(), '0.0%');
      await page.evaluate(() => window.scrollTo(0, 0)); await audit('frozen-version-independent-market');
      await page.screenshot({ path: path.join(outputDir, `full-app-review-versions-${width}.png`), fullPage: true });
      await page.locator('[data-review-version-picker] select').selectOption('UNKNOWN');
      await page.getByRole('button', { name: 'HAD 胜平负', exact: true }).click();
      assert.equal(await page.locator('[data-review-overview-rate]').textContent(), '100.0%');
      assert.ok((await page.locator('[data-review-version-picker]').textContent()).includes('完整历史有 1 场版本未追溯'));
      await audit('legacy-untraced-version-not-reassigned');
      mode = 'formal-zero';
