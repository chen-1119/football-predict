...
check('low SP with model/market contradiction is rejected', !lowSpWeak.eligible
  && lowSpWeak.blockers.includes('market-implied-probability-contradiction')
  && lowSpWeak.blockers.includes('negative-expected-value')
  && lowSpWeak.blockers.includes('low-sp-without-model-edge'), { result: lowSpWeak });
...
check('long price without probability separation remains observation-only', !highSpWeak.eligible
  && highSpWeak.blockers.includes('long-price-probability-too-low')
  && highSpWeak.blockers.includes('model-separation-too-thin'), { result: highSpWeak });
...
check('policy contains no max-SP direction switch', MULTI_FACTOR_POLICY_VERSION === 'multi-factor-market-evidence-v2'
  && MIN_MODEL_GAP === 0.06
...
check('sync invalidates cached pre-match rows when the multi-factor policy changes',
  syncSource.includes('sporttery-day-formula-trace-v65-trusted-incremental-history')
