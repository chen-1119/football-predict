...
try {
  const current = readJson(path.join(sourceDataDir, "matches-current.json"))
    .filter((row) => SOURCE_IDS.has(sourceId(row)))
    .map((row) => {
  ...
  const ambiguitySource = readJson(path.join(sourceDataDir, "matches-current.json"))
  ...
  const historyClockSource = readJson(path.join(sourceDataDir, "matches-current.json"))
    .find((row) => sourceId(row) === "2040650");
