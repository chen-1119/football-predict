...
const run = async () => {
  ...
  const atomicTransitionDetailIndex = currentPayloadSource.indexOf('source: "sqlite-atomic"');
  ...
  check("atomic SQLite transition snapshot bypasses immutable history JSON", (
    /\? atomicSqliteSnapshot\.available\s+\? Promise\.resolve\(\{ source: "sqlite-atomic", rows: \[\] \}\)/.test(currentPayloadSource)
