...
check("live SQLite prebuild creates a transient rollback snapshot and keeps the normal outage path O(1)", () => {
  ...
  assert.match(productionCloneVerifier, /production clone has transactional SQLite sidecar data before verification/);
  assert.match(productionCloneVerifier, /production clone has transactional SQLite sidecar data after verification/);
  assert.match(productionCloneVerifier, /--transaction-child/);
  assert.match(productionCloneVerifier, /spawnSync\(process\.execPath/);
  assert.match(productionCloneVerifier, /sidecarsContainNoTransactionalData/);
