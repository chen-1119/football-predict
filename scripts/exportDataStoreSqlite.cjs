...
const historyMatchesPayload = loadBaseProjection ? readCoreJson("matches-history.json", null) : null;
const syncMeta = loadBaseProjection ? readCoreJson("sync-meta.json", null) : null;
const externalSignals = loadBaseProjection ? readCoreJson("external-signals.json", null) : null;
const oddsHistory = loadBaseProjection ? readCoreJson("odds-history.json", null) : null;
const predictionSnapshots = loadBaseProjection ? readCoreJson("prediction-snapshots.json", null) : null;
...
const materializeArchiveProjection = (rows) => (
  Array.isArray(rows)
    ? attachArchivedPreMatchPredictions(
        rows,
        predictionSnapshots,
...
const currentMatches = materializeArchiveProjection(currentMatchesPayload);
const historyMatches = materializeArchiveProjection(historyMatchesPayload);
...
const predictionUpserter = (db) => {
  const select = db.prepare("SELECT * FROM prediction_snapshots WHERE id = ?");
  const insert = insertPredictionStateStatement(db, "prediction_snapshots");
  return (state) => {
    const existingRow = select.get(state.id);
    const merged = mergeCanonicalPredictionStates(existingRow ? storedPredictionState(existingRow) : null, state);
...
const exportIncrementalRows = async (db) => {
  ...
  for (const [dataset, rows] of [["current", currentMatches], ["history", historyMatches]]) {
    ...
    for (const match of rows) {
      const normalizedMatch = normalizeLegacyReviewClock(match);
      const sourceMatchId = normalizedMatch?.sourceMatchId || sourceMatchIdFor(normalizedMatch?.id) || null;
      const id = `${dataset}:${normalizedMatch?.id || sourceMatchId || hashPayload(normalizedMatch)}`;
      ...
      if (dataset === "history" && (guardedFinal || guardedHistoryIds.has(id))) {
  ...
  const publicOddsRows = Array.isArray(oddsHistory?.rows) ? oddsHistory.rows.slice(-oddsLimit) : [];
  for (const row of publicOddsRows) addOddsCandidate(row, "public");
  imported.publicOddsRows = publicOddsRows.length;

  const publicPredictionRows = Array.isArray(predictionSnapshots?.rows)
