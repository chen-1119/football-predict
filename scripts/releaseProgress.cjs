...
function evaluateReleaseProgress(observation) {
  ...
  const transactionComplete = statusStable === true && identityValid && clocksValid && status?.status === "complete"
    && status.ok === "1" && status.exitCode === "0" && markers.app === sha && markers.liveComplete === sha && recoveryPending === false;
  let state = "unknown";
  ...
  return {
    ...
    productionWrites: 0, sourceValidationExecuted: false, liveAcceptanceProven: false,
    nextAction: state === "running" ? "observe-existing-process" : transactionComplete ? "run-live-acceptance"
...
function collectReleaseProgress(sha) {
  if (!/^[a-f0-9]{64}$/.test(sha)) throw new Error("invalid release SHA");
  ...
  const before = read(statusPath, 4096);
  ...
  const after = read(statusPath, 4096);
  return { sha, checkedAt: new Date().toISOString(), status, statusStable: before?.text === after?.text,
    processes, markers, recoveryPending, services, logTail: log?.text || "",
