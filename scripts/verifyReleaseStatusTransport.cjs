...
check("preflight and recovery-helper failures remain blockers", () => {
  assert.ok(source.includes('blockers.push("remote release preflight failed")'));
  assert.ok(source.includes('blockers.push("remote cold-recovery helper does not match the candidate bundle")'));
  assert.ok(source.includes("actualSha256 === expectedSha256"));
});
