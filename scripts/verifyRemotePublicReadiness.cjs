...
const sqlitePrimaryReady = (health) => {
  const sqlite = sqliteFromHealth(health);
  const currentRead = currentReadFromHealth(health);
  return sqlite.available === true && currentRead.source === "sqlite";
};
...
const waitForSqlitePrimary = async (initialHealth) => {
  if (!requireSqlite || sqlitePrimaryReady(initialHealth)) {
  ...
  for (let attempt = 2; attempt <= sqliteReadyAttempts; attempt += 1) {
    ...
    if (sqlitePrimaryReady(health)) return { health, attempts: attempt };
  }
...
const run = async () => {
  ...
  pushCheck(checks, "AI survival frontend chunk uses the protected v2 publication", aiArenaAssets.length > 0
    && aiArenaAssets.every(({ response }) => response.status === 200)
    && aiArenaAssets.some(({ response }) => response.text?.includes("/api/v1/ai-arena"))
    && aiArenaAssets.some(({ response }) => response.text?.includes("ai-big-five-survival-v2"))
  ...
  pushCheck(checks, "scheduled matches have BEST and atomically bound HHAD directions",
    recommendationCoverage.version === "current-recommendation-coverage-v5"
      ...
      && Number(recommendationCoverage.missingDirectionMatches || 0) === 0
      && Number(recommendationCoverage.watchMatches || 0) === 0
  ...
  pushCheck(checks, "sqlite read source when required", !requireSqlite || (sqlite.available === true && currentRead.source === "sqlite"), {
  ...
  const redundancyCollectorCount = Number(
    sourceHealth.body?.officialSourceRedundancy?.trustedCollectorCount,
  );
  pushCheck(checks, "trusted collector redundancy counts independent runtimes, not keys",
    collectorEvidence?.version === "trusted-market-collector-evidence-v1"
      ...
      && redundancyCollectorCount === trustedCollectorCount, {
      ...
      trustedCollectorCount: Number.isFinite(trustedCollectorCount) ? trustedCollectorCount : null,
      independenceDomainCount: independenceDomains.length,
  ...
  pushCheck(checks, "candidate prospective cutoff heartbeat is live and unblocked",
    modelEvaluation.status === 200
      ...
      && candidateCaptureHeartbeat?.freshnessLimitMs === 120_000
      ...
      && Number(candidateCaptureHeartbeat?.intervalSeconds || 0) >= 15
  ...
  const aiArenaStatusValid = aiArenaStatus.status === 200
    ...
    && (
      aiArenaState !== "LOCKED"
      || (
        aiArenaStatus.body?.complete === true
        && Number(aiArenaStatus.body?.availableMatches) === 10
