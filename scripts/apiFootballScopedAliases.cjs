...
const VERSION = "api-football-scoped-aliases-v1-20260907";
...
].map(row => Object.freeze({ ...row, provider: "api-football", leagueId: 140, season: 2026 })));
...
function scopedTeamAliases(match, side, fixture, localLeagueAliases) {
  if (!["home", "away"].includes(side) || !Array.isArray(localLeagueAliases)
    || !localLeagueAliases.includes("la liga")) return [];
  ...
  return SCOPED_ALIASES.filter(row => row.localName === match?.[`${side}TeamName`]
