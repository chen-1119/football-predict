...
const now = Number.isFinite(Date.parse(process.env.AUDIT_NOW || ""))
  ? Date.parse(process.env.AUDIT_NOW)
  : Date.now();
...
const businessDate = (match) => {
  ...
  return new Date(kickoff + 8 * 60 * 60 * 1000).toISOString().slice(0, 10);
};
const oddsBucket = (odds) => {
  if (!Number.isFinite(odds)) return "unknown";
  if (odds <= 1.45) return "<=1.45";
  if (odds <= 1.70) return "1.46-1.70";
  if (odds <= 2.05) return "1.71-2.05";
  if (odds <= 2.60) return "2.06-2.60";
  return ">2.60";
};
...
const formalArchivedBest = (match) => {
  ...
  if (
    ...
    || !Number.isFinite(Number(prediction.odds))
...
const summarize = (rows) => {
  ...
  return {
    ...
    won,
    lost,
    hitRate: settled ? Number((won / settled).toFixed(4)) : null,
    averageOdds: settled
      ? Number((rows.reduce((sum, row) => sum + row.odds, 0) / settled).toFixed(3))
      : null,
  };
};
const group = (rows, keyFn) => Object.fromEntries(
  [...new Set(rows.map(keyFn).filter(Boolean))].sort().map((key) => [key, summarize(rows.filter((row) => keyFn(row) === key))])
);
...
const rows = (Array.isArray(history) ? history : [])
  ...
  .map((match) => {
    ...
    return {
      ...
      sourceMatchId: text(match.sourceMatchId),
      date: businessDate(match),
      kickoffTime: match.kickoffTime || null,
      league: text(match.leagueName || match.leagueId) || "unknown",
      market: prediction.oddsPoolCode,
      tip: prediction.tipCode,
      odds: Number(prediction.odds),
      oddsBucket: oddsBucket(Number(prediction.odds)),
      evidenceScore: finite(prediction?.multiFactorEvidence?.evidenceScore ?? prediction?.trustScore),
      result,
    };
  })
  .filter(Boolean);
...
for (const days of [7, 14, 30, 90]) {
  ...
  const scoped = rows.filter((row) => {
    const at = Date.parse(row.kickoffTime || `${row.date}T00:00:00+08:00`);
    return Number.isFinite(at) && at >= cutoff && at <= now;
  });
  windows[`${days}d`] = {
    ...summarize(scoped),
    byMarket: group(scoped, (row) => row.market),
    byOddsBucket: group(scoped, (row) => row.oddsBucket),
    byTip: group(scoped, (row) => `${row.market}:${row.tip}`),
    byLeague: group(scoped, (row) => row.league),
  };
}

const output = {
  version: "recommendation-quality-audit-v1",
  generatedAt: new Date(now).toISOString(),
  historyFile,
  sampleRows: rows.length,
  windows,
  policy: {
    source: "immutable archived pre-match BEST recommendations only",
    postMatchDirectionReconstruction: false,
    intendedUse: "diagnostic and conservative gate tuning",
  },
};
