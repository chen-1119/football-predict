...
function assertUnitsDrained(states){for(const [unit,s]of Object.entries(states))if(s.LoadState!=='not-found'){const noProcess=(unit.endsWith('.timer')&&(s.MainPID===undefined||s.MainPID===''))||s.MainPID==='0'||s.MainPID===0;assert.ok(['inactive','failed'].includes(s.ActiveState)&&noProcess,'writer not drained: '+unit);}}
function stopUnits(states){for(const [unit,s]of Object.entries(states))if(s.LoadState!=='not-found')run('/usr/bin/systemctl',['stop',unit],{timeout:120000});assertUnitsDrained(unitStates());}
...
function recoveredIncidentAnchor(incident){
 const dir=ROOT+'/'+policy.RECOVERY_ANCHOR_SHA,prior=load(dir,{fresh:false});assertSameInput(prior.observation,incident);assert.equal(prior.observation.worker.admissionAnchor,undefined);
 for(const name of ['activation-started.json','failed.json','recovered.json'])regular(dir+'/'+name);
 for(const name of ['writers-paused.json','backed-up.json','backup','code-swap-started.json','production-input.json','production-projection-started.json','production-projection.json','proof-accepted.json','activated.json','accepted.json'])assert.equal(fs.existsSync(dir+'/'+name),false,'prior repair progressed beyond the fixed no-swap recovery: '+name);
 const started=read(dir+'/activation-started.json'),failed=read(dir+'/failed.json'),recovered=read(dir+'/recovered.json');assert.equal(failed.error,'writer not drained: football-cleanup.timer');assert.equal(recovered.mode,'operator-before-code-swap');assert.equal(recovered.capsuleSha256,policy.RECOVERY_ANCHOR_SHA);assert.equal(recovered.failedAt,failed.checkedAt);assert.equal(recovered.reason,'timer-units-do-not-expose-MainPID');assert.equal(recovered.codeRestored,true);assert.equal(recovered.dataRestored,false);assert.equal(recovered.servicesRestored,true);assert.equal(recovered.entrypointGuardPreserved,true);assert.equal(recovered.runtimeSha256,prior.observation.runtime);assert.deepEqual(recovered.verifiedBeforeFileHashes,prior.observation.files);
 const clocks=[started.checkedAt,failed.checkedAt,recovered.checkedAt,new Date().toISOString()].map(Date.parse);assert.ok(clocks.every(Number.isFinite)&&clocks.every((v,i)=>i===0||v>=clocks[i-1]));
 const units=Object.entries(started.units).filter(([,s])=>s.ActiveState==='active').map(([u])=>u).sort();assert.deepEqual([...recovered.restoredUnits].sort(),units);assert.deepEqual(Object.keys(recovered.statuses).sort(),units);const current=unitStates();for(const unit of units){assert.equal(recovered.statuses[unit],'active');assert.equal(current[unit].ActiveState,'active');}
 assert.equal(digest(policy.ENTRYPOINT),prior.entrypointGuard.sha256);if(fs.existsSync(ROOT+'/current'))assert.notEqual(read(ROOT+'/current').capsuleSha256,policy.RECOVERY_ANCHOR_SHA,'prior repair recovery marker not cleared');
 const admissionEvidence=prior.observation.worker.admissionEvidence;assert.ok(admissionEvidence);return{version:'r773-no-swap-recovery-anchor-v1',capsuleSha256:policy.RECOVERY_ANCHOR_SHA,signatureSha256:digest(dir+'/capsule.sig'),activationSha256:digest(dir+'/activation-started.json'),failureSha256:digest(dir+'/failed.json'),recoverySha256:digest(dir+'/recovered.json'),recoveredAt:recovered.checkedAt,incidentInputHash:policy.incidentInputHash(incident),noWriteStages:true,servicesRestored:true,originalWorkerPid:prior.observation.worker.pid,originalInvocationId:prior.observation.worker.evidence.invocationId,admissionEvidence};
}
function faultObservation(incident){
 ...
 const binding={pid:s.pid,invocationId,startedAt:s.lastCycle.startedAt,finishedAt:s.lastCycle.finishedAt};let mode='admission-failure',evidence,admissionEvidence,admissionAnchor;
 if(s.lastError?.command==='npm'&&JSON.stringify(s.lastError.args)==='["run","postgres:sync"]')evidence=boundFaultLogs(logs,binding);
  ...
  if(prior.some(row=>row._SYSTEMD_INVOCATION_ID===invocationId))admissionEvidence=boundFaultLogs(prior,{...binding,startedAt:incident.generation.committedAt,finishedAt:s.lastCycle.startedAt});
  else admissionAnchor=recoveredIncidentAnchor(incident);
 }
 assert.ok(Date.parse(s.checkedAt)>=Date.parse(String(x.serviceAfter.ExecMainStartTimestamp).replace(/\bCST$/,'+0800')));
 return{pid:s.pid,mode,faultCode:'POSTGRES_REFERENCE_ADMISSION_LIMIT',statusCheckedAt:s.checkedAt,lastCycleStartedAt:s.lastCycle.startedAt,lastCycleFinishedAt:s.lastCycle.finishedAt,evidence,...(admissionEvidence?{admissionEvidence}:{}),...(admissionAnchor?{admissionAnchor}:{})};
}
async function observe({requireFault=true,requireWindow=true}={}){
 ...
 assert.equal(archive.ok,true);const window=requireWindow?currentWindow():null;
 ...
 const observation={checkedAt:new Date().toISOString(),runtime,complete,generation:context.pointer,previousGeneration,snapshot:objects.snapshot,postgres:{...postgres,publication:objects.publication},frontendStateSha256:digest('/var/lib/football-release/frontend-state.json'),entrypoint:{path:policy.ENTRYPOINT,sha256:policy.hash(entryBytes),base64:entryBytes.toString('base64')},faultCode:requireFault?'POSTGRES_REFERENCE_ADMISSION_LIMIT':null,worker:null,window,archive:{...archive,baselineRows:manifest.baseline.records.length},files,frozen:{rootHash:objectHash(objects.recommendations),count:objects.recommendations.length,publicDecisionsRoot:objectHash(objects.publicDecisions),publicDecisionCount:objects.publicDecisions.length,publicEvidenceRoot:objectHash(objects.publicEvidence),publicEvidenceCount:objects.publicEvidence.length},frozenRecords:{recommendations:objects.recommendations,publicDecisions:objects.publicDecisions,publicEvidence:objects.publicEvidence,archives:objects.archives},productionWrites:0};observation.worker=requireFault?faultObservation(observation):null;observation.checkedAt=new Date().toISOString();return observation;
}
...
module.exports={observe,prepare,activate,recover,load,assertSameInput,assessOfficial,boundFaultLogs,captureReferenceObjects,tree,unitStates,assertUnitsDrained,projectProduction};
