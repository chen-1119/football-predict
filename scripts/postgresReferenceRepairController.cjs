...
function projectProduction(dir,p){
 const proof=read(dir+'/isolated-proof.json');assert.equal(proof.ok,true);assert.equal(proof.productionWrites,0);assert.equal(proof.capsuleSha256,path.basename(dir));
 ...
 const result=read(dir+'/production-projection.json');assert.equal(result.ok,true);assert.equal(result.productionProjectionWrites,true);assert.equal(result.archiveSha256,proof.archiveSha256);assert.equal(result.frozenPreserved,true);return result;
}
