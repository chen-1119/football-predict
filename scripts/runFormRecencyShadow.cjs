...
function run(inputFile, outputFile) {
  ...
  const implementationFiles = [__filename, path.join(__dirname, "formRecencyShadow.cjs"), path.join(__dirname, "syncData.cjs"), path.join(root, "src/services/modelInputUsage.cjs"), path.join(root, "src/services/strictInstant.cjs")];
