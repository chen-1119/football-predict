...
async function runFixture() {
  ...
  try {
    ...
    await candidate.query("UPDATE football.uuid_parent SET label='candidate changed' WHERE id=$1", [parentId]);
    await candidate.query("UPDATE football.uuid_child SET label='candidate changed' WHERE id=$1", [childId]);
    ...
    console.log(JSON.stringify({ ok: true, verifier: "postgres-mirror-uuid-and-type-allowlist", copiedRows: first.copiedRows,
      repairedRows: repaired.copiedRows, warmCopiedRows: warm.copiedRows, unsupportedTypeRejected: true, productionWrites: 0 }));
  } finally {
