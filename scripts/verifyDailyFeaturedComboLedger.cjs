...
const candidate = (id, odds, evidenceScore, market = "HAD", tipCode = "1") => ({
  matchId: `sporttery_${id}`, sourceMatchId: id,
  ...
  market, tipCode, handicapLine: market === "HHAD" ? -1 : 0, odds, evidenceScore,
  modelProbability: 0.6, // Explicit synthetic model estimate, not evidenceScore / 100.
});
const candidates = [candidate("1", 1.62, 82), candidate("2", 1.58, 80), candidate("3", 2.02, 77), candidate("4", 1.70, 75)];
const two = choose(candidates, 2);
assert.ok(two); assert.equal(two.size, 2); assert.ok(two.totalOdds >= 2.5); assert.equal(two.legs.length, 2);
const three = choose(candidates, 3);
assert.ok(three); assert.equal(three.size, 3); assert.ok(three.totalOdds >= 5); assert.equal(three.legs.length, 3);
assert.equal(choose([candidate("1", 1.2, 90), candidate("2", 1.2, 90)], 2), null);
const entry = { id: "combo:test", size: 2, businessDate: "2026-09-16", frozenAt: "2026-09-16T13:00:00.000Z",
  legs: [candidate("1", 1.62, 82), candidate("2", 1.58, 80)], settlement: { status: "PENDING", settledAt: null } };
const history = [
...
assert.equal(settled.settlement.status, "WON"); assert.equal(settled.settlement.results.every((leg) => leg.result === "WON"), true);
...
assert.equal(clock.date, "2026-09-16"); assert.equal(clock.hour, 21);
...
assert.equal(denied.entries.length,1); assert.deepEqual(denied.entries[0].legs,entry.legs); assert.equal(denied.publicPayload.publishable,false);
