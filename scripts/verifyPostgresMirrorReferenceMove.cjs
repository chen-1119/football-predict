...
(async()=>{
  ...
  const schema="CREATE SCHEMA football; CREATE TABLE football.publications(publication_id text PRIMARY KEY,state text NOT NULL); CREATE UNIQUE INDEX one_current ON football.publications(state) WHERE state='current'; CREATE TABLE football.frozen_recommendations(id text PRIMARY KEY,publication_id text NOT NULL REFERENCES football.publications(publication_id),payload json NOT NULL); CREATE TABLE football.prediction_snapshots(id text PRIMARY KEY,payload json); CREATE TABLE football.projection_meta(key text PRIMARY KEY,value text); CREATE TABLE football.post_match_reviews(id text PRIMARY KEY,match_id text NOT NULL,decision_id text NOT NULL,payload json NOT NULL,UNIQUE(match_id,decision_id));";
  try{await source.query(schema);await source.query("CREATE DATABASE "+name);const target=new URL(url);target.pathname="/"+name;candidate=new Pool({connectionString:target.href,max:1,ssl:false});await candidate.query(schema);
    ...
    await source.query("INSERT INTO football.post_match_reviews VALUES('source-review','match-1','decision-1','{ \"score\": 1 }');");
    ...
    await run();await candidate.query("INSERT INTO football.publications VALUES('candidate-only','previous'); UPDATE football.frozen_recommendations SET publication_id='candidate-only';");await candidate.query("UPDATE football.post_match_reviews SET id='candidate-review';");const repaired=await run();
    assert.equal(repaired.ok,true);assert.equal(repaired.removedCandidateRows,2);assert.equal(repaired.copiedRows,2);assert.deepEqual((await candidate.query("SELECT * FROM football.post_match_reviews")).rows,(await source.query("SELECT * FROM football.post_match_reviews")).rows);
    ...
    console.log(JSON.stringify({ok:true,verifier:'mirror-retained-child-reference-move',copiedRows:2,removedCandidateParents:1,removedConflictingLeafRows:1,frozenPayloadUnchanged:true,productionWrites:0}));
