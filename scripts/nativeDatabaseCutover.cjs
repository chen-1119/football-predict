...
// Before the final mirror, prevent a new connection from committing a write
// between the last source snapshot and the rename. Existing publisher-owned
// sessions remain usable; the caller must drain every other session and keep
// the generation commit barrier. The durable v4 journal must already exist,
// so recovery can reopen the bound active database after an interrupted fence.
async function fenceNativeDatabaseConnections(client, contract) {
  validate(contract);
  let begun = false;
  try {
    begun = true; await client.query("BEGIN; SET LOCAL lock_timeout='2s'; SET LOCAL statement_timeout='10s'");
    assert.equal((await client.query("SELECT pg_try_advisory_xact_lock(hashtext('football-native-database-cutover-v1')) AS locked")).rows[0].locked, true);
    const before = await topology(client, contract);
    assert.equal(before.football?.oid, contract.oldDatabaseOid, "cannot fence a changed active database");
    assert.equal(before[contract.candidateDatabase]?.oid, contract.newDatabaseOid, "cannot fence a changed candidate database");
    assert.equal(before[contract.archiveDatabase], undefined);
    assert.equal(before.football.owner, "football"); assert.equal(before[contract.candidateDatabase].owner, "football");
    await client.query("ALTER DATABASE football ALLOW_CONNECTIONS false");
    await client.query("ALTER DATABASE " + quote(contract.candidateDatabase) + " ALLOW_CONNECTIONS false");
    const after = await topology(client, contract);
    assert.equal(after.football.oid, contract.oldDatabaseOid); assert.equal(after[contract.candidateDatabase].oid, contract.newDatabaseOid);
    assert.equal(after.football.allows_connections, false); assert.equal(after[contract.candidateDatabase].allows_connections, false);
    await client.query("COMMIT"); begun = false;
    return { ok: true, oldDatabaseOid: contract.oldDatabaseOid, newDatabaseOid: contract.newDatabaseOid,
      newConnectionsBlocked: true, existingConnectionsTerminated: 0, modelDataWrites: 0 };
  } catch (error) {
    if (begun) try { await client.query("ROLLBACK"); } catch { /* Recovery observes actual catalog state after uncertain commit. */ }
    throw error;
  }
}
module.exports = { switchNativeDatabase, fenceNativeDatabaseConnections };
