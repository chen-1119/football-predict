...
const validateRecoveryRow = (row) => {
  ...
  if (!predictionOdds) errors.push("prediction-odds-invalid");
  if (!selectedOdds || Math.abs(selectedOdds - predictionOdds) > 0.000001) {
  ...
  if (!String(row?.signature || "").includes(`BEST:${marketPool}:${tipCode}:`)) {
  ...
  if (!String(evidence?.sourceCycleId || "").startsWith("sporttery-full-sync:")) {
