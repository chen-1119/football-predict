...
(async () => {
  ...
  try {
    for (const width of [390, 768, 1440]) {
      ...
      const interval = page.locator('[data-review-uncertainty]');
      assert.equal(await interval.getAttribute('open'), null);
      assert.ok((await interval.locator('summary').textContent()).includes('独立同概率假设'));
      const allInterval = await page.locator('[data-review-interval]').textContent();
      await interval.locator('summary').focus(); await page.keyboard.press('Enter');
      assert.equal(await interval.getAttribute('open'), '');
      await audit('uncertainty-keyboard-open');
      await page.screenshot({ path: path.join(outputDir, `review-ui-uncertainty-${width}.png`), fullPage: true });
      ...
      assert.notEqual(await page.locator('[data-review-interval]').textContent(), allInterval);
      ...
      await page.getByRole('button', { name: /Data references/ }).click();
      assert.ok((await page.locator('[data-review-uncertainty] summary').textContent()).includes('Not a forecast'));
      await page.locator('[data-review-uncertainty] summary').focus(); await page.keyboard.press('Enter');
      await audit('english-reference-uncertainty');
      await page.close();
    }
