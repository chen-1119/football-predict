...
const host = process.env.RELEASE_STATUS_HOST || process.env.RELEASE_DEPLOY_HOST || "170.106.75.73";
const user = process.env.RELEASE_STATUS_USER || process.env.RELEASE_DEPLOY_USER || "ubuntu";
const sshPort = Number(process.env.RELEASE_STATUS_PORT || process.env.RELEASE_DEPLOY_PORT || 18789);
...
const checkRemoteRecoveryHelper = (ssh, expectedSha256) => {
  ...
  return {
    ...
    expectedSha256,
    actualSha256,
    matchesCandidate: actualSha256 === expectedSha256,
    stderrTail: "",
    error: null,
    evidenceSource: ssh.aggregate?.version || null
  };
...
const run = async () => {
  ...
  const remoteRecoveryHelper = checkRemoteRecoveryHelper(ssh, bundle.recoveryHelperSha256);
  ...
  const canAttemptDeploy = bundle.ok && ssh.ok && remotePreflight.ok && remoteRecoveryHelper.ok;
  ...
  if (ssh.ok && !remoteRecoveryHelper.ok) blockers.push("remote cold-recovery helper does not match the candidate bundle");
  ...
  if (!sqliteReady) blockers.push("public origin has not switched to sqlite");
