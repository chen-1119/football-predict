...
async function projectAndVerify({root,dir,input,policy,pool,scope,target,production,candidate}){
 ...
 const initial=require(root+'/server/dataGenerationStore.cjs').resolveCurrentGeneration({storeDir:input.store});assert.deepEqual(initial.pointer,input.expectedGeneration);
  ...
  const result=await require(root+'/scripts/postgresProjectionSync.cjs').syncPostgresProjectionFromSource(source,{pool,mode:'backfill',force:true,aiArenaPath:root+'/public/data/ai-arena.json'});
  assert.equal(result.ok,true);assert.notEqual(result.skipped,true);const projected=await publication(pool);for(const k of ['generationId','manifestHash','sourceCycleId','committedAt'])assert.equal(projected[k],input.expectedGeneration[k]);
  const {protectedFrozenRecommendationHash}=require(root+'/scripts/nativeReleaseDataPlane.cjs');
  const recommendations=(await pool.query('SELECT decision_id,to_jsonb(f)::text AS record FROM football.frozen_recommendations f ORDER BY decision_id COLLATE "C"')).rows.map(r=>({id:r.decision_id,hash:protectedFrozenRecommendationHash(r.record)}));
  const manifest=(await pool.query('SELECT payload::text AS payload FROM football.source_snapshots WHERE id=$1',[api.INDEX_ID])).rows[0];assert.ok(manifest);const index=JSON.parse(manifest.payload);assert.equal(index.rowCount,input.expectedFrozen.publicDecisions.length);
  const archive=(await pool.query("SELECT payload->>'version' AS version,payload->>'contentHash' AS content_hash,payload->>'evidenceContentHash' AS evidence_hash,json_array_length(payload->'rows') AS records,json_array_length(payload->'evidence') AS evidence,encode(sha256(convert_to(payload::text,'UTF8')),'hex') AS payload_sha256 FROM football.source_snapshots WHERE id=$1",[api.SOURCE_ID])).rows[0];
  ...
  return {ok:true,capsuleSha256:input.capsuleSha256,checkedAt:new Date().toISOString(),elapsedMs:Date.now()-start,projectionSkipped:false,frozenPreserved:preserved.ok,archiveIndexVerified:true,archiveRecords:index.rowCount,referenceRoot:index.rootHash,archiveSha256:archive.payload_sha256,projection:result,productionProjectionWrites:scope==='production-repair',productionWrites:scope==='production-repair'?1:0,isolatedDatabaseWrites:scope==='isolated-candidate',providerRequests:0,maxRssKiB:process.resourceUsage().maxRSS,postgresMemory:memory.finish()};
...
async function main(){
 ...
 if(scope==='production-repair'){
  ...
  assert.ok(fs.existsSync(dir+'/code-swap-started.json'));const proofBytes=fs.readFileSync(dir+'/isolated-proof.json'),accepted=JSON.parse(fs.readFileSync(dir+'/proof-accepted.json')),proof=JSON.parse(proofBytes);assert.equal(accepted.sha256,hash(proofBytes));assert.equal(proof.ok,true);assert.equal(proof.capsuleSha256,input.capsuleSha256);assert.equal(proof.productionWrites,0);assert.equal(input.isolatedArchiveSha256,proof.archiveSha256);assert.equal(hash(fs.readFileSync(p.entrypointGuard.path)),p.entrypointGuard.sha256);
