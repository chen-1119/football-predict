...
(async () => {
  ...
  pushCheck("06-c-end-release-experience", "release freeze keeps the candidate deadline heartbeat live and fail-closed", hasAll(bundleReleaseScript, [
    ...
    "football-release-deadline-capability-$$-${RANDOM}",
    "RuntimeMaxSec=15s",
    "active candidate deadline-only capability probe failed closed",
    "prepare_legacy_deadline_compat_runtime",
    "cleanup_legacy_deadline_compat_runtime",
    "run_legacy_candidate_capture_refresh",
    "/run/football-release-deadline-compat.XXXXXX",
    "BENCHMARK_PROSPECTIVE_LEDGER_FILE=${LEGACY_DEADLINE_COMPAT_RUNTIME_DIR}",
    "BENCHMARK_PROSPECTIVE_CAPTURE_STATUS_FILE=${LEGACY_DEADLINE_COMPAT_RUNTIME_DIR}",
    "CANDIDATE_PROSPECTIVE_CHALLENGER_SUITE_FILE=${LEGACY_DEADLINE_COMPAT_RUNTIME_DIR}",
    "CANDIDATE_PROSPECTIVE_TEMPERATURE_NEUTRALIZATION_SUITE_FILE=${LEGACY_DEADLINE_COMPAT_RUNTIME_DIR}",
    "CANDIDATE_COMMON_COHORT_SHADOW_G2_FILE=${LEGACY_DEADLINE_COMPAT_RUNTIME_DIR}",
    "CANDIDATE_COMMON_COHORT_SHADOW_G2_V2_FILE=${LEGACY_DEADLINE_COMPAT_RUNTIME_DIR}",
    "active post-swap candidate capture does not advertise deadline-only support",
    "legacy-deadline-capture-refresh",
    ...
    "candidate deadline capture heartbeat refresh process failed (exit=%s)",
    "--capture-once",
    "--uid=football",
    ...
    "rollback fail-stop: legacy deadline compatibility runtime could not be cleaned",
    "fail-stop: legacy deadline compatibility runtime could not be cleaned from EXIT trap"
  ...
  pushCheck("06-c-end-release-experience", "remote list detail and frozen archive share one recommendation direction", (
    hasAll(verifyRemoteRecommendationParity, [
      ...
      "every scheduled row that exposes a direction has an explicit BEST recommendation",
      "every post-kickoff or result-phase row that exposed a direction has an immutable pre-match archive",
