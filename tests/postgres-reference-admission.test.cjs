...
test('explicit aggregate/item bounds fail instead of returning partial or silently empty arrays',()=>{
 const p=payload([fixture()]),c=contextFor(p);assert.throws(()=>read(c,{maxRetainedChars:10}),{code:'POSTGRES_REFERENCE_ADMISSION_LIMIT'});assert.throws(()=>read(c,{maxItems:1}),{code:'POSTGRES_REFERENCE_ADMISSION_LIMIT'});assert.throws(()=>read(c,{maxRetainedChars:257*1024*1024}),/invalid native reference admission bound/);assert.equal(read(c).publicReferenceEvidence.length,1);
});
...
test('unique bound evidence over 128 MiB materializes and validates with bounded 768 MiB heap',()=>{
 ...
 try{fs.writeSync(fd,'{"updatedAt":'+JSON.stringify(auditAt)+',"retentionDays":31,"publicReferenceEvidence":[');for(let i=0;i<65;i++){const f=fixture({id:String(885000+i),mutateSource:source=>{source.probabilityModel.padding='x'.repeat(2*1024*1024);}}),entry=JSON.stringify(f.entry);records.push(f.record);fs.writeSync(fd,(i?',':'')+entry);evidenceHash.update((i?',':'')+entry);}evidenceHash.update(']');fs.writeSync(fd,'],"publicReferenceDecisions":'+JSON.stringify(records)+',"rows":[]}');}finally{fs.closeSync(fd);}
 ...
 const child=spawnSync(process.execPath,['--max-old-space-size=768','-e',program,contextPath],{encoding:'utf8',timeout:60000,windowsHide:true,maxBuffer:1024*1024});assert.equal(child.status,0,child.stderr);const expectedHash=sha(JSON.stringify(records)),expectedEvidence=evidenceHash.digest('hex');assert.deepEqual(JSON.parse(child.stdout),{records:65,evidence:65,archiveHash:expectedHash,evidenceHash:expectedEvidence,indexRows:65,archiveVersion:expectedHash,evidenceVersion:expectedEvidence});
});
