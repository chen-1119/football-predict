...
function mount(){
  ...
  const app={language:'zh',setLanguage:()=>{},currentUser:{username:'完整账户名称'.repeat(8)},logout:()=>{},dataSync:{currentLoaded:true,currentCount:12,recommendationReliable:false,lastCheckedAt:'2026-09-21T02:03:00Z',sourceUpdatedAt:'2026-09-21T02:00:00Z',liveUpdates:'poll',refreshIntervalSeconds:30}};
  ...
  vm.runInNewContext(source,{module,exports:module.exports,require:id=>id==='react'?react:id==='react/jsx-runtime'?{jsx:(type,props)=>({type,props}),jsxs:(type,props)=>({type,props})}:id==='lucide-react'?new Proxy({},{get:(_,key)=>String(key)}):id==='../context/AppContextCore'?{useApp:()=>app}:(()=>{throw Error(id);})(),document,window,Date,Intl});
