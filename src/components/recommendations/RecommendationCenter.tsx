...
function Pick({d,settlement,handicapSettlement,quality,language,onSelectMatch}:{d:Decision;settlement:Settlement;handicapSettlement?:Settlement|null;quality?:SelectionQuality|null;language:Language;onSelectMatch:(id:string)=>void}){
  const zh=language==='zh';
  return <article className="rc-pick" data-decision-id={d.decisionId} data-record-hash={d.recordHash}><header><span>{d.matchNo||d.sourceMatchId} · {format(d.kickoffTime,language)}</span><span className={`rc-state rc-state--${settlement.state}`}>{resultLabel(settlement.state,zh)}</span></header>
...
export function RecommendationCenter({language,onSelectMatch,mode='recommendations',initialTab='single',selectedTab,onTabChange}:Props){
  ...
  const setTab=(next:RecommendationTab)=>{setLocalTab(next);onTabChange?.(next);};
  ...
  return <section className="recommendation-center" aria-labelledby="rc-title">
    ...
    {!loading&&(tab==='single'?stale:!comboFresh||quoteExpired)&&<p className="rc-notice" role="status">{tab!=='single'&&!review?comboWaitingMessage:(zh?'当前显示已发布快照，行情更新延迟；旧快照不作为新串关输入。':'Published snapshots are retained while inputs are delayed; old snapshots do not create new combos.')}</p>}
