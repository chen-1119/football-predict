...
function hadSelectionBadge(selectionClass:ReturnType<typeof primarySelectionSummary>['had']['selectionClass'],zh:boolean){
  if(selectionClass==='draw-value')return zh?'价值平局':'Value draw';
  if(selectionClass==='underdog-value')return zh?'价值冷门':'Value upset';
  if(selectionClass==='model-draw')return zh?'模型平局':'Model draw';
  if(selectionClass==='model-underdog')return zh?'模型非热门':'Model nonfavorite';
  return null;
}
function PrimaryPickHeader({d,language}:{d:Decision;language:Language}){
  ...
  return <div className="rc-primary-picks" aria-label={zh?'胜平负首选与让球延伸':'Primary 1X2 pick and handicap extension'}>
    <div className="rc-primary-pick rc-primary-pick--had" data-had-selection={summary.had.selectionClass||'legacy'}><span>{zh?'胜平负首选':'1X2 primary'}{hadSelectionBadge(summary.had.selectionClass,zh)&&<em className="rc-had-badge">{hadSelectionBadge(summary.had.selectionClass,zh)}</em>}</span><strong>{title(summary.had.code,zh)}</strong><small>SP {summary.had.odds.toFixed(2)} · {(summary.had.probability*100).toFixed(1)}%</small></div>
...
function OutcomeResearchPanel({research,selectionClass,language,now,quoteObservedAt,cutoffTime,kickoffTime}:{research:OutcomeCategoryResearch|null|undefined;selectionClass?:string|null;language:Language;now:number;quoteObservedAt:string;cutoffTime:string;kickoffTime:string}){
  if(!research)return null;
  const zh=language==='zh',candidate=research.outcomes.find(row=>row.code===research.candidateCode);
  const formallySelected=selectionClass==='draw-value'||selectionClass==='underdog-value';
  const quoteAt=Date.parse(quoteObservedAt),quoteFresh=Number.isFinite(quoteAt)&&now>=quoteAt&&now-quoteAt<=15*60000;
  const open=now<Math.min(Date.parse(cutoffTime),Date.parse(kickoffTime)),active=research.researchQualified&&quoteFresh&&open;
  const favoriteValueWarning=outcomeResearchFavoriteValueWarning(research,zh);
  return <section className="rc-outcome-research" data-outcome-category={research.category} data-research-qualified={active} aria-label={zh?'胜平负分类研究':'1X2 category study'}>
    <div className="rc-outcome-research__heading"><div><small>{zh?'胜平负分类研究 · 非正式推荐':'1X2 category study · not a formal pick'}</small><strong>{outcomeCategoryLabel(research.category,zh)}</strong></div><span>{active&&candidate?(zh?`${title(candidate.code,true)} · 研究候选`:`${title(candidate.code,false)} · study candidate`):!open?(zh?'已截止 · 仅供复盘':'Cutoff passed · review only'):!quoteFresh?(zh?'报价过期 · 观望':'Price expired · watch'):favoriteValueWarning??(candidate?(zh?`${title(candidate.code,true)} · 分类观察`:`${title(candidate.code,false)} · category observation`):(zh?'证据不足 · 观望':'Evidence limited · watch'))}</span></div>
    <div className="rc-outcome-research__grid">{research.outcomes.map(item=><div key={item.code} className={item.code===research.candidateCode?(active?'is-candidate':'is-observation'):undefined}><strong>{title(item.code,zh)}</strong><span>{zh?'模型':'Model'} {(item.modelProbability*100).toFixed(1)}% · {zh?'市场去水':'Market fair'} {(item.fairMarketProbability*100).toFixed(1)}%</span><small>SP {item.odds.toFixed(2)} · {zh?'模型估值':'Model EV'} {item.expectedValue>=0?'+':''}{(item.expectedValue*100).toFixed(1)}%</small></div>)}</div>
    <p>{formallySelected?(zh?'本场已通过价值方向选择器发布为正式胜平负方向；这里保留三方向概率与价格对照，用于复盘它是否真的优于热门。':'This match was formally published through the value-selection path; the three-way probability/price comparison remains for review against the favorite.'):active&&candidate?(zh?'这是当前报价下的分类研究候选，不单独覆盖正式方向。':'This is a category research candidate at the frozen price and does not independently override the formal pick.'):candidate?(zh?'该方向只是分类观察，证据未达到价值选择门槛。':'This outcome is a category observation below the value-selection gate.'):(zh?'本场只展示三方向比较，不根据热门或高 SP 强行改选。':'The three outcomes are compared without forcing the favorite or a higher-SP alternative.')}{research.reasons.length?` ${zh?'限制：':'Limits: '}${research.reasons.slice(0,2).map(reason=>outcomeResearchReasonLabel(reason,zh)).join(zh?'、':'; ')}。`:''}</p>
  </section>;
}
function RecordDetails({d,selection,language,onSelectMatch}:{d:Decision;selection?:ComboSelection;language:Language;onSelectMatch:(id:string)=>void}){
  ...
  return <details className="rc-details"><summary><ChevronDown size={14} aria-hidden="true"/>{zh?'分析依据与冻结版本':'Evidence & frozen version'}</summary>
    <div><p>{selection?.market==='HHAD'?(zh?'本腿选取完整让球概率中最高的方向，未使用“胜平负先命中”的条件占比；因此可以与单场卡片的伴随方向不同。盘口和SP均取自本条冻结记录。':'This leg uses the highest unconditional handicap probability, not the share conditional on a correct 1X2 pick. It may differ from the companion pick. The handicap and SP are archived with this selection.'):(d.hadSelection?.mode==='market-dislocation'?(zh?'本场胜平负来自价值方向选择：该方向不是模型绝对第一概率，但相对同节点竞彩去水概率具有明显正偏差、正模型EV，且与模型第一方向差距受限。串关引用同一冻结方向。':'This 1X2 pick comes from the value-selection path: it is not the model probability leader, but it has a material positive edge versus the same frozen de-vigged market, positive model EV, and a bounded gap to the model leader. Combos reuse the same frozen direction.'):(zh?'胜平负首选取自完整模型概率的最大项；串关从同一决策中选择胜平负或让球玩法，每场只入选一腿。':'The 1X2 pick is the maximum of the model vector. A combo selects 1X2 or handicap from the same decision, with one leg per match.'))}</p>
...
function Pick({d,supplementarySettlement,settlement,handicapSettlement,quality,outcomeResearch,language,onSelectMatch,reviewSelection,now}:{d:Decision;supplementarySettlement?:SingleRow['supplementarySettlement'];settlement:Settlement;handicapSettlement?:Settlement|null;quality?:SelectionQuality|null;outcomeResearch?:OutcomeCategoryResearch|null;language:Language;onSelectMatch:(id:string)=>void;reviewSelection?:ReviewedSingleRow;now:number}){
  const zh=language==='zh';
  ...
  return <article className="rc-pick" data-decision-id={d.decisionId} data-record-hash={d.recordHash} data-selection-status={reviewSelection?.selectedMarket==='HHAD'?'diagnostic':quality?.status??'reference'} data-price-status={reviewSelection?.selectedMarket==='HHAD'?'unknown':selectionPriceStatus(quality)}><header><span>{d.matchNo||d.sourceMatchId} · {format(d.kickoffTime,language)} · {reviewSelection?.selectedMarket==='HHAD'?(zh?'让球归档诊断':'Archived handicap diagnostic'):selectionReferenceLabel(quality,language)}</span><span className={`rc-state rc-state--${selected?.state||'PENDING'}`}>{selected?resultLabel(selected.state,zh):(zh?'待赛果':'Pending')}</span></header>
    ...
    {!reviewSelection&&<OutcomeResearchPanel research={outcomeResearch} selectionClass={d.hadSelection?.selectionClass} language={language} now={now} quoteObservedAt={d.quoteObservedAt} cutoffTime={d.cutoffTime} kickoffTime={d.kickoffTime}/>}
    ...
    <details className="rc-analysis"><summary><span>{zh?'概率与让球分析':'Probabilities & handicap analysis'}</span><ChevronDown size={16} aria-hidden="true"/></summary><div className="rc-analysis__body">
      ...
      <SupplementaryResearchNote research={d.supplementaryResearch} settlement={supplementarySettlement} language={language}/>
    </div></details>
    <RecordDetails d={d} language={language} onSelectMatch={onSelectMatch}/>
  </article>;
}
function ComboCard({combo,settlement,language,onSelectMatch,reviewSelection,currentAdvisory}:{combo:Combo;settlement?:Settlement;language:Language;onSelectMatch:(id:string)=>void;reviewSelection?:ReviewedComboRow;currentAdvisory?:ComboRow['currentAdvisory']}){
  const zh=language==='zh';
  return <article className="rc-combo"><header><div><small>{zh?'参考串关 · 模型未验证':'Reference combo · model unvalidated'}</small><h3>{combo.size}{zh?'串1':'-leg combo'}</h3></div><div><strong>SP {combo.totalOdds.toFixed(2)}</strong><small>{zh?'下限':'Minimum'} {combo.size===2?'2.50':'5.00'}</small></div></header>
    ...
    {!!currentAdvisory?.length&&<p className="rc-notice" role="note" data-frozen-combo-conflict="true">{zh?`当前赛前参考与这条已冻结组合的 ${currentAdvisory.length} 腿胜平负方向相反；${currentAdvisory.some(item=>!item.knownAtFreeze)?'至少一条分歧在冻结后才记录。':'分歧在冻结时已记录。'}原组合与 SP 保留供复盘，当前仅作观望。`:`A pre-match reference now conflicts with ${currentAdvisory.length} frozen 1X2 leg(s). ${currentAdvisory.some(item=>!item.knownAtFreeze)?'At least one conflict was recorded after freezing.':'The conflict was recorded by freeze time.'} The original combo and SP remain for review; watch only.`}</p>}
    ...
    <p className="rc-disclaimer">{zh?'组合按模型概率在合格玩法中排序，SP 只用于最低门槛；不是低 SP 必胜，也未验证价格优势或真实串关命中率。每场只选一腿。':'Eligible legs are ranked by model probability; SP only sets the minimum total. Neither low SP nor this ranking proves price value or combo accuracy. One leg per match.'}</p>
  </article>;
}
...
function HitRateTargetReview({report,language}:{report:ModelQualityReport;language:Language}){
  const target=report.hitRateTarget;if(!target)return null;
  const zh=language==='zh',pct=(value:number|null)=>value===null?'—':`${(value*100).toFixed(1)}%`;
  const goal=`${Math.round(target.targetHitRate*100)}%`;
  return <details className="rc-review-insights" data-hit-rate-target="observational"><summary><div><strong>{zh?`${goal}目标进度`:`${goal} target progress`}</strong><span>{zh?`按模型版本 · 截至 ${target.asOfBusinessDate}`:`By model version · through ${target.asOfBusinessDate}`}</span></div><ChevronDown size={18} aria-hidden="true"/></summary>
    <div className="rc-review-insights__body"><p>{zh?`目标命中率 ${goal}；下列为真实已结算胜平负单场成绩，不含串关、双选或待赛果。按竞彩日统计每场最后一次赛前发布，版本分开，不随列表筛选变化。`:`Target hit rate ${goal}; actual settled published 1X2 singles below exclude combos, dual choices and pending results. Each event uses its last pre-match publication, grouped by match day and model version, independent of list filters.`}</p>
      {target.byModelVersion.length?target.byModelVersion.map(group=><section className="rc-performance" key={group.modelVersion}><header><strong>{group.modelVersion}</strong></header><div className="rc-performance__rows">{(['last7','last30'] as const).map(key=>{
        const row=group.windows[key],status=row.settled===0?(zh?'待积累赛果':'Awaiting results'):!row.sampleSufficient?(row.numericTargetReached?(zh?'数值达标 · 样本不足':'Numeric target met · too few samples'):(zh?'未达目标 · 样本不足':'Below target · too few samples')):!row.numericTargetReached?(zh?'未达数值目标':'Below numeric target'):!row.evidenceSufficient?(zh?'数值达标 · 证据不足':'Numeric target met · insufficient evidence'):(zh?'观察证据达标 · 未经独立验证':'Observational evidence met · not independently validated');
        return <article key={key} data-target-window={key}><div><strong>{key==='last7'?(zh?'近7日':'Last 7 days'):(zh?'近30日':'Last 30 days')}</strong><small>{row.from} — {row.through}</small><small>{status}</small></div><div><strong>{pct(row.hitRate)}</strong><small>{zh?'命中 / 已结算':'Won / settled'} {row.won} / {row.settled} · {zh?'待赛果':'Pending'} {row.pending}</small><small>{zh?'独立竞彩日':'Independent match days'} {row.independentMatchDays} · {zh?'95%区间':'95% interval'} {row.hitRateInterval95?`${pct(row.hitRateInterval95.lower)}–${pct(row.hitRateInterval95.upper)}`:'—'}</small></div></article>;
      })}</div></section>):<p>{zh?'尚无可按模型版本核验的发布样本。':'No verifiable published samples by model version yet.'}</p>}
      <small>{zh?`观察证据要求至少 ${target.minimumSettled} 场已结算、${target.minimumMatchDays} 个独立竞彩日，且95%区间下限达到 ${goal}。数值达标不代表模型通过独立验证，也不会自动转为正式推荐。`:`Observational evidence requires ${target.minimumSettled} settled events, ${target.minimumMatchDays} independent match days and a 95% interval lower bound of ${goal}. Reaching the numeric target does not validate the model or promote formal recommendations.`}</small>
    </div>
  </details>;
}
function SpBandReview({report,language}:{report:ModelQualityReport;language:Language}){
  if(!report.bySpBucket)return null;
  const zh=language==='zh';
  const labels:Record<string,string>={sp_le_1_45:'SP ≤ 1.45',sp_gt_1_45_le_1_70:'1.45 < SP ≤ 1.70',sp_gt_1_70_le_2_05:'1.70 < SP ≤ 2.05',sp_gt_2_05_le_2_60:'2.05 < SP ≤ 2.60',sp_gt_2_60:'SP > 2.60'};
  const pct=(value:number|null)=>value==null?'—':`${(value*100).toFixed(1)}%`;
  return <details className="rc-review-insights" data-sp-band-review="observational"><summary><div><strong>{zh?'全量胜平负单场 SP 分组':'All published 1X2 picks by SP band'}</strong><span>{zh?`同一批已结算比赛 ${report.settled} 场，不随页面筛选变化；不是提高 SP 的选场规则`:`Same ${report.settled} settled matches, unaffected by page filters; not a higher-SP selection rule`}</span></div><ChevronDown size={18} aria-hidden="true"/></summary><div className="rc-review-insights__body"><div className="rc-performance__rows">{Object.entries(labels).map(([key,label])=>{const group=report.bySpBucket?.[key];return <article key={key}><div><strong>{label}</strong><small>{zh?'样本 / 命中':'Samples / wins'} {group?.settled??0} / {group?.won??0} · {zh?'命中率':'Hit rate'} {pct(group?.hitRate??null)}</small></div><div><strong>{zh?'单位注净值':'Flat-stake net'} {group?.flatStakeNetUnits==null?'—':group.flatStakeNetUnits.toFixed(2)}</strong><small>Brier {zh?'模型':'model'} {group?.brier?.toFixed(3)??'—'} / {zh?'同期市场':'same-event market'} {group?.marketBrier?.toFixed(3)??'—'}</small></div></article>;})}</div><p>{zh?'以上仅为已冻结方向的观察性分组；SP 不是概率本身。模型期望值为负的场次同样保留在复盘里，样本不足时不改变发布权重或强制选择冷门。':'These are observational groups of frozen directions. SP is not a probability. Negative model-EV rows remain in review; small samples do not change weights or force underdogs.'}</p>{report.byModelPriceSignal&&<p>{zh?'模型价格诊断（非校准收益预测）':'Model price arithmetic (not calibrated return prediction)'}：{zh?'期望值为负':'negative EV'} {report.byModelPriceSignal.negative?.settled??0} {zh?'场':'matches'} · {zh?'非负':'nonnegative'} {report.byModelPriceSignal.nonnegative?.settled??0} {zh?'场':'matches'}</p>}</div></details>;
}
function SelectionClassReview({report,language}:{report:ModelQualityReport;language:Language}){
  if(!report.bySelectionClass)return null;
  const zh=language==='zh',labels:Record<string,string>={
    'market-favorite':zh?'市场热门':'Market favorite',
    'model-draw':zh?'模型平局':'Model draw',
    'model-underdog':zh?'模型非热门':'Model nonfavorite',
    'draw-value':zh?'价值平局':'Value draw',
    'underdog-value':zh?'价值冷门':'Value upset',
    legacy:zh?'旧口径':'Legacy',
  };
  const pct=(value:number|null)=>value==null?'—':`${(value*100).toFixed(1)}%`;
  return <details className="rc-review-insights" data-selection-class-review="observational"><summary><div><strong>{zh?'正路 / 平局 / 冷门分组复盘':'Favorite / draw / upset review'}</strong><span>{zh?'只统计真实冻结方向，不为多样化强行补冷门':'Frozen published picks only; no diversity quota'}</span></div><ChevronDown size={18} aria-hidden="true"/></summary><div className="rc-review-insights__body"><div className="rc-performance__rows">{Object.entries(labels).map(([key,label])=>{const group=report.bySelectionClass?.[key];return <article key={key}><div><strong>{label}</strong><small>{zh?'样本 / 命中':'Samples / wins'} {group?.settled??0} / {group?.won??0}</small></div><div><strong>{pct(group?.hitRate??null)}</strong><small>Brier {group?.brier?.toFixed(3)??'—'} · {zh?'单位注净值':'flat net'} {group?.flatStakeNetUnits==null?'—':group.flatStakeNetUnits.toFixed(2)}</small></div></article>;})}</div><p>{zh?'价值平局/冷门只有通过冻结模型与同节点去水市场的价值门槛才进入正式样本。样本少时只观察，不据此调整当天方向配额。':'Value draws/upsets enter the formal cohort only through the frozen model-versus-market value gate. Small samples remain observational and never set daily direction quotas.'}</p></div></details>;
}
function SupplementaryPerformance({stats,language}:{stats?:SupplementarySummary;language:Language}){
 if(!stats)return null;
 const zh=language==='zh';
 return <section className="rc-review-windows" aria-label={zh?'比分与总进球冻结研究统计':'Frozen score and total-goals research statistics'}>
  {(['exactScore','totalGoals'] as const).map(key=>{const row=stats[key];return <div key={key}>
   <span>{key==='exactScore'?(zh?'同向比分研究':'Aligned score study'):(zh?'总进球研究':'Total-goals study')}</span>
   <strong>{row.hitRate==null?'—':`${(row.hitRate*100).toFixed(1)}%`}</strong>
   <small>{zh?'命中 / 已结算':'Hits / settled'} {row.won} / {row.settled} · {zh?'待赛果':'Pending'} {row.pending}</small>
  </div>;})}
  <p>{zh?`全台账独立研究统计（不随下方筛选变化）。仅统计发布时冻结的首选；${stats.excludedWithoutFrozenPicks} 条缺少冻结首选的记录未计入。未绑定官方 SP，不计算收益率，模型尚未验证。`:`Separate all-ledger study (unaffected by filters below). Only picks frozen at publication count; ${stats.excludedWithoutFrozenPicks} records without frozen picks are excluded. No official SP, no ROI, model unvalidated.`}</p>
 </section>;
}
...
export function RecommendationCenter({language,onSelectMatch,mode='recommendations',initialTab='single',selectedTab,onTabChange}:Props){
  ...
  const priceUnsupported=!review&&currentDay?data?.current.filter(row=>selectionPriceStatus(row.selectionQuality)==='unsupported').length??0:0;
  ...
  return <section className="recommendation-center" aria-labelledby="rc-title">
    ...
    {!review&&tab==='single'&&priceUnsupported>0&&<p className="rc-notice rc-notice--price" role="note" data-price-unsupported={priceUnsupported}>{zh?`价格提示：今天 ${priceUnsupported} / ${data?.current.length??0} 场冻结模型方向的“模型概率 × SP − 1”为负。入选不等于价格已验证；这些方向仅供复盘研究。`:`Price check: ${priceUnsupported} / ${data?.current.length??0} frozen model directions have negative model probability × SP − 1. Reference admission does not validate price; these directions remain research references.`}</p>}
    {!review&&tab==='single'&&data&&<section className="rc-dual-research-list" aria-label={zh?'双选研究':'Dual-choice study'}>
      ...
      {data.todayDualResearch?.length?data.todayDualResearch.map(row=><DualResearchV2 key={row.id} row={row} language={language} onSelectMatch={onSelectMatch}/>):<div className="rc-empty"><strong>{zh?'今日暂无合格双选研究':'No eligible dual-choice study today'}</strong><p>{zh?'等待当天比赛的赛前模型证据与新鲜、完整的官方赔率；缺少胜平负不会单独阻止让球双选。':'Awaiting verified pre-match model evidence and fresh complete official SP. Missing 1X2 alone does not block a handicap pair.'}</p></div>}
    </section>}
    <Stats value={summary} zh={zh} scoped={review} diagnostic={review&&tab==='single'&&filters.market==='HHAD'}/>
    {review&&<ReviewWindows seven={reviewPage.data?.summary.windows.last7} thirty={reviewPage.data?.summary.windows.last30} language={language}/>}
    {review&&tab==='single'&&modelQuality&&<HitRateTargetReview report={modelQuality} language={language}/>}
    {tab==='single'&&modelQuality&&<><div className="rc-empty" data-model-quality="unvalidated"><strong>{zh?'模型仍在验证中':'Model validation in progress'}</strong><p>{zh?'同场比较：模型':'Same-event comparison: model'} {modelQuality.hitRate==null?'—':`${(modelQuality.hitRate*100).toFixed(1)}%`} · {zh?'市场热门':'Market favorite'} {modelQuality.marketTopHitRate==null?'—':`${(modelQuality.marketTopHitRate*100).toFixed(1)}%`} · {modelQuality.settled}{zh?'场已结算':' settled'}</p><p>Brier {zh?'模型':'model'} {modelQuality.brier?.toFixed(3)??'—'} / {zh?'同期市场':'same-event market'} {modelQuality.marketBrier?.toFixed(3)??'—'} · LogLoss {modelQuality.logLoss?.toFixed(3)??'—'} / {modelQuality.marketLogLoss?.toFixed(3)??'—'}</p><small>{zh?`目前覆盖 ${modelQuality.independentMatchDays} 个有赛果竞彩日；评估要求至少 ${modelQuality.minimumSettled} 场、${modelQuality.minimumMatchDays} 日，并优于同场市场概率。缺赛果不计入命中率，当前报告不是独立回测。`:`${modelQuality.independentMatchDays} settled match days; review requires ${modelQuality.minimumSettled} events, ${modelQuality.minimumMatchDays} days and better probability metrics than the same-event market. Missing results are excluded; this is observational, not a held-out backtest.`}</small></div><SpBandReview report={modelQuality} language={language}/><SelectionClassReview report={modelQuality} language={language}/></>}
    {review&&tab==='single'&&<SupplementaryPerformance stats={data?.review.statistics.supplementary} language={language}/>}
    {review&&tab==='single'&&<details className="rc-review-insights"><summary><div><strong>{zh?'让球复盘与模型校准':'Handicap review & calibration'}</strong><span>{zh?'按版本查看命中率、样本分母与验证结果':'Compare versions, denominators and validation results'}</span></div><ChevronDown size={18} aria-hidden="true"/></summary><div className="rc-review-insights__body"><HandicapPerformance breakdown={data?.review.statistics.handicapBreakdown} legacy={handicapSummary} zh={zh}/><HandicapCalibrationPanel profile={data?.review.handicapCalibration} language={language}/></div></details>}
