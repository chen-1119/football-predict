...
function OutcomeResearchPanel({research,language,now,quoteObservedAt,cutoffTime,kickoffTime}:{research:OutcomeCategoryResearch|null|undefined;language:Language;now:number;quoteObservedAt:string;cutoffTime:string;kickoffTime:string}){
  ...
  return <section className="rc-outcome-research" data-outcome-category={research.category} data-research-qualified={active} aria-label={zh?'胜平负分类研究':'1X2 category study'}>
    ...
    <p>{active&&candidate?(zh?'这是当前报价下的分类候选，尚未通过独立比赛日验证，不会替换上方已发布方向或进入串关。':'This is a category study candidate at the frozen price. It has not passed independent match-day validation and does not replace the published pick or enter combos.'):candidate?(zh?'该方向只是分类观察，证据未达到研究候选门槛；不会替换已发布方向或进入串关。':'This outcome is a category observation below the study gate; it does not replace the published pick or enter combos.'):(zh?'本场只展示三方向比较，不根据热门或高 SP 强行改选。':'The three outcomes are compared without forcing the favorite or a higher-SP alternative.')}{research.reasons.length?` ${zh?'限制：':'Limits: '}${research.reasons.slice(0,2).map(reason=>outcomeResearchReasonLabel(reason,zh)).join(zh?'、':'; ')}。`:''}</p>
  </section>;
}
