...
function HandicapCalibrationPanel({profile,language}:{profile?:HandicapCalibrationProfile;language:Language}){
  ...
  return <section className="rc-calibration"><header><div><span>{zh?'盘口强度分组复盘':'Handicap calibration by line'}</span><strong>{profile?.sampleRows??0}{zh?'个主方向命中样本':' HAD-hit frozen samples'}</strong></div><small>{zh?'只有时间前推验证通过的分组才自动影响新让球方向。':'Only holdout-validated groups can change new handicap picks.'}</small></header>
