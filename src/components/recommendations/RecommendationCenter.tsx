...
function HandicapBlock({d,settlement,language}:{d:Decision;settlement?:Settlement|null;language:Language}){
  ...
  return <section className="rc-handicap">
    ...
    <p>{handicapNarrative(d,zh)}</p>{primarySelectionSummary(d).handicap?.status==='pass'&&<small className="rc-handicap__warning">{zh?'顶部延伸建议：不追让球。完整三项概率继续保留，用于判断深盘风险。':'Top extension: pass the handicap. Full three-way probabilities remain as deep-line risk diagnostics.'}</small>}{h.probabilityBasis==='conditional-on-straight-primary'&&<small className="rc-handicap__basis">{zh?'以下三项为“胜平负首选成立”条件下的净胜球占比，不是独立HHAD命中率。':'The three shares below are conditional on the 1X2 thesis landing; they are not standalone HHAD hit probabilities.'}</small>}{h.overallTipCode&&h.overallTipCode!==h.tipCode&&<small className="rc-handicap__diagnostic">{zh?'独立HHAD全局最高项':'Standalone HHAD top'}：{handicapTitle(h.overallTipCode,zh)} · {zh?'未作为伴随首选':'not used as companion pick'}</small>}{h.historicalCalibration?.applied&&<small className="rc-handicap__learned">{zh?'历史盘口校准已启用':'Historical handicap calibration active'} · {h.historicalCalibration.key}</small>}
...
function PrimaryPickHeader({d,language}:{d:Decision;language:Language}){
  ...
  return <div className="rc-primary-picks" aria-label={zh?'胜平负首选与让球延伸':'Primary 1X2 pick and handicap extension'}>
    ...
    <div className={`rc-primary-pick rc-primary-pick--hhad ${h?.status==='pass'?'is-pass':''}`}><span>{zh?'让球延伸':'Handicap extension'}</span>{h?h.status==='recommend'?<><strong>{h.lineText} · {handicapTitle(h.code!,zh)}</strong><small>{h.conditional?(zh?'条件占比 ':'Conditional share '):''}{(h.probability!*100).toFixed(1)}%{h.odds?(' · SP '+h.odds.toFixed(2)):''}{h.calibrated?(zh?' · 已校准':' · calibrated'):''}</small></>:<><strong>{h.lineText} · {zh?'不追让球':'Pass handicap'}</strong><small>{zh?'盘口模型偏 ':'Model leans '}{handicapTitle(h.riskCode!,zh)} {(h.riskProbability!*100).toFixed(1)}%{h.suggestedCode?(zh?' · 同向备选 ':' · aligned alternative ')+handicapTitle(h.suggestedCode,zh)+' '+(h.suggestedProbability!*100).toFixed(1)+'%':''}</small></>:<><strong>—</strong><small>{zh?'等待有效让球线与净胜球数据':'Awaiting valid handicap inputs'}</small></>}</div>
  </div>;
}
