...
import { quoteSourceLabel, comboLaneFresh, comboPreviewForSize, comboLegSelection, primarySelectionSummary, handicapExtensionText, handicapAnalysisBasis, calibrationSampleBasis, sameDirectionConcentration, type Decision, type Settlement, type Combo, type ComboSelection, type Summary, type Outcome, type HandicapCalibrationProfile, type HandicapBreakdown, type ModelQualityReport } from '../../services/recommendationCenterView';
...
const selectionPriceStatus=(quality:SelectionQuality|null|undefined)=>!quality||quality.expectedValue==null?'unknown':quality.expectedValue<0?'unsupported':'model-supported';
...
function Pick({d,settlement,handicapSettlement,quality,language,onSelectMatch,reviewSelection}:{d:Decision;settlement:Settlement;handicapSettlement?:Settlement|null;quality?:SelectionQuality|null;language:Language;onSelectMatch:(id:string)=>void;reviewSelection?:ReviewedSingleRow}){
  ...
  return <article className="rc-pick" data-decision-id={d.decisionId} data-record-hash={d.recordHash} data-selection-status={reviewSelection?.selectedMarket==='HHAD'?'diagnostic':quality?.status??'reference'} data-price-status={reviewSelection?.selectedMarket==='HHAD'?'unknown':selectionPriceStatus(quality)}><header><span>{d.matchNo||d.sourceMatchId} · {format(d.kickoffTime,language)} · {reviewSelection?.selectedMarket==='HHAD'?(zh?'让球归档诊断':'Archived handicap diagnostic'):!quality?(zh?'模型方向 · 证据待核':'Model direction · evidence pending'):quality.qualified===false?(zh?'观望方向':'Watch direction'):selectionPriceStatus(quality)==='unsupported'?(zh?'模型方向 · 价格不支持':'Model direction · price unsupported'):(zh?'输入达标方向':'Input-checked direction')}</span><span className={`rc-state rc-state--${selected?.state||'PENDING'}`}>{selected?resultLabel(selected.state,zh):(zh?'待赛果':'Pending')}</span></header>
...
export function RecommendationCenter({language,onSelectMatch,mode='recommendations',initialTab='single',selectedTab,onTabChange}:Props){
  ...
  return <section className="recommendation-center" aria-labelledby="rc-title">
    ...
    {!review&&tab==='single'&&priceUnsupported>0&&<p className="rc-notice rc-notice--price" role="note" data-price-unsupported={priceUnsupported}>{zh?`价格提示：今天 ${priceUnsupported} / ${data?.current.length??0} 场冻结模型方向的“模型概率 × SP − 1”为负。输入达标不等于价格支持；这些方向仅供复盘研究。`:`Price check: ${priceUnsupported} / ${data?.current.length??0} frozen model directions have negative model probability × SP − 1. Input readiness is not price support; these directions remain research references.`}</p>}
