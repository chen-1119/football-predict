...
import { primarySelectionSummary, quoteSourceLabel } from '../../services/recommendationCenterView';
...
export function PublishedMatchPick({row,language,loading=false,failed=false,compact=false,now=Date.now()}:Props){
  ...
  const d=row.decision,s=primarySelectionSummary(d),h=s.handicap;
  ...
  return <div className={`published-match-pick${compact?' is-compact':''}`} data-decision-id={d.decisionId} data-record-hash={d.recordHash}>
    <div className="published-match-pick__directions">
      ...
      <div><small>{h&&!h.conditional?(zh?'让球分析':'Handicap analysis'):(zh?'让球伴随方向':'Companion handicap')}</small><strong>{h?`${h.lineText} · ${publishedPickLabel(h.code,language,true)}`:'—'}</strong><span>{h?`${h.conditional?(zh?'条件占比 ':'Conditional share '):''}${(h.probability*100).toFixed(1)}%${h.odds?' · SP '+h.odds.toFixed(2):''}`:(zh?'等待有效让球数据':'Awaiting handicap data')}</span></div>
    </div>
