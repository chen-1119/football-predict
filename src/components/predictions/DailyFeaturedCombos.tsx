...
const ComboCard: React.FC<{
  ...
  const title = language === 'zh'
    ? `${combo.size} 场精选分析`
    : `${combo.size}-match featured analysis`;
  const floor = language === 'zh'
    ? `总 SP ≥ ${combo.minimumTotalOdds.toFixed(2)}`
    : `Total SP ≥ ${combo.minimumTotalOdds.toFixed(2)}`;
    ...
    <article className={`daily-combo-card ${combo.status === 'ready' ? 'is-ready' : 'is-empty'}`}>
      ...
      {combo.status === 'ready' ? (
        <>
          <div className="daily-combo-card__legs">
            {combo.legs.map((leg, index) => {
              ...
              return (
                <button
                  key={`${combo.size}-${leg.match.id}`}
                  type="button"
                  className="daily-combo-leg"
                  onClick={() => onSelectMatch(leg.match.id)}
                >
        ...
        <div className="daily-combo-card__empty">
          ...
          <p>
            {language === 'zh'
              ? '当前没有同时满足质量门槛和总 SP 下限的组合，不强行凑单。'
              : 'No combination currently clears both the quality gate and SP floor.'}
          </p>
        </div>
      )}
    </article>
...
export const DailyFeaturedCombos: React.FC<DailyFeaturedCombosProps> = ({
  ...
  const combos = React.useMemo(
    () => enabled ? buildDailyFeaturedCombos(matches) : null,
    [enabled, matches]
  );
      ...
      <header className="daily-combo-board__header">
        <div>
          ...
          <div>
            ...
            <p>
              {language === 'zh'
                ? '只使用当前正式赛前方向；优先证据强、总 SP 刚过门槛的组合。'
                : 'Uses current formal pre-match picks only, preferring strong evidence and totals just above the required floor.'}
            </p>
          </div>
        </div>
        <span className="daily-combo-board__rule">
          {language === 'zh' ? '2场 ≥2.50 · 3场 ≥5.00' : '2-leg ≥2.50 · 3-leg ≥5.00'}
        </span>
      </header>

      {!enabled || !combos ? (
        <div className="daily-combo-board__disabled">
          {language === 'zh'
            ? '正式推荐池尚未达到可发布状态，精选组合保持暂停。'
            : 'The formal recommendation pool is not publishable yet; featured combinations remain paused.'}
        </div>
      ) : (
