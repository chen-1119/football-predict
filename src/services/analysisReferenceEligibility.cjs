...
const isDirection = (value) => value === "1" || value === "X" || value === "2";
...
const DIRECTIONAL_REFERENCE_MIN_EVIDENCE_SCORE = 52;
const DIRECTIONAL_REFERENCE_MIN_MODEL_GAP = 0.03;
const DIRECTIONAL_REFERENCE_MIN_EXPECTED_VALUE = -0.01;
const DIRECTIONAL_REFERENCE_MIN_DATA_QUALITY = 0.50;
...
const isPreMatchReferenceWindowOpen = (match, now) => {
...
const isCalibratedMarketAnalysisReferenceEligible = (match, prediction, now = Date.now()) => {
...
const isDirectionalAnalysisReferenceEligible = (match, prediction, now = Date.now()) => {
  if (!prediction || !isPreMatchReferenceWindowOpen(match, now)) return false;
  ...
  const blockers = Array.isArray(evidence?.blockers)
    ? evidence.blockers.filter((value) => typeof value === "string")
    : [];
...
const isModelOnlyAnalysisReferenceEligible = (match, prediction, now = Date.now()) => {
  if (!prediction || !isPreMatchReferenceWindowOpen(match, now)) return false;
  if (prediction.marketType !== "BEST") return false;
  if (prediction.recommendationAction !== "reference") return false;
...
module.exports = {
  DIRECTIONAL_REFERENCE_MIN_DATA_QUALITY,
  DIRECTIONAL_REFERENCE_MIN_EVIDENCE_SCORE,
  DIRECTIONAL_REFERENCE_MIN_EXPECTED_VALUE,
  DIRECTIONAL_REFERENCE_MIN_MODEL_GAP,
