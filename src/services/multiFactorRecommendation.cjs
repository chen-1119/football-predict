const MULTI_FACTOR_POLICY_VERSION = 'multi-factor-market-evidence-v2';
...
const evaluateMultiFactorRecommendation = (input = {}) => {
  ...
  const minimumModelProbability = odds !== null && odds <= 1.45
    ? 0.56
    : odds !== null && odds <= 2.2
      ? 0.46
      : odds !== null && odds <= 3
  ...
  if (expectedValue !== null && expectedValue < -0.04) blockers.push('negative-expected-value');
  ...
  // Very short SP must still show real model/market value. A popular outcome is
  // not automatically a good recommendation.
  if (odds !== null && odds <= 1.45) {
    if (probabilityEdge === null || probabilityEdge < 0.015) blockers.push('low-sp-without-model-edge');
    if (expectedValue === null || expectedValue < 0.01) blockers.push('low-sp-without-value');
  }

  // Longer prices are allowed, but only with unusually strong independent
  // evidence and value. There is no blanket max-SP direction switch.
  if (odds !== null && odds >= 2.8) {
    if (modelProbability === null || modelProbability < 0.4) blockers.push('long-price-probability-too-low');
    if (probabilityEdge === null || probabilityEdge < 0.035) blockers.push('long-price-edge-too-thin');
    if (expectedValue === null || expectedValue < 0.04) blockers.push('long-price-value-too-thin');
  }
