...
/**
 * Conservative promotion guard derived from settled historical buckets.
 * It only tightens formal recommendation eligibility; it never changes the
 * underlying probability vector or manufactures a different direction.
 *
 * The current production history has shown materially weaker settlement in
 * HHAD and higher-SP buckets. Until rolling calibration has a healthy sample
 * again, these lanes require stronger independent evidence or remain reference
 * only. This is intentionally one-way cooling, not automatic loosening.
 */
const evaluateHistoricalRecommendationGuard = (input = {}) => {
  ...
  // Draws remain eligible when evidence is genuinely strong; do not balance
  // directions artificially. The extra guard only prevents weak high-SP draws.
