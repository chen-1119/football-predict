...
export interface HandicapAnalysis {
  ...
  probabilities:Record<Outcome,number>; modelProbability:number; modelGap:number; exactMargin:number; exactMarginProbability:number;
...
export interface RecommendationCenterData {
  ...
  review:{singles:SingleRow[];combos:ComboRow[];limit:number;statistics:{single:Summary;handicap?:Summary;two:Summary;three:Summary};definition:string};
...
function handicapAnalysis(v:unknown):HandicapAnalysis{
  ...
  return {version:'handicap-margin-v1',market:'HHAD',handicapLine,handicapLineText:text(h.handicapLineText),tipCode,probabilities,modelProbability,
    ...
    calibration:'unvalidated',marketReference};
}
