...
const COMBO_MINIMUMS: Record<FeaturedComboSize, number> = {
  2: 2.5,
  3: 5,
};

const isCandidate = (match: Match, prediction: PredictionDetail | undefined): prediction is PredictionDetail => {
  if (!prediction || match.status !== 'SCHEDULED') return false;
  if (prediction.marketType !== 'BEST' || prediction.recommendationAction !== 'recommend') return false;
  if (prediction.oddsPoolCode !== 'HAD' && prediction.oddsPoolCode !== 'HHAD') return false;
  if (!['1', 'X', '2'].includes(prediction.tipCode)) return false;
  return true;
};

const candidateForMatch = (match: Match): FeaturedComboLeg | null => {
  const prediction = getFormalRecommendationPrediction(match);
  if (!isCandidate(match, prediction)) return null;
  ...
  // Daily featured combinations are deliberately stricter than the ordinary
  // single-match display. Historically weak HHAD/high-SP lanes need stronger
  // evidence before they are allowed to influence a multi-leg plan.
...
const combinationRank = (legs: FeaturedComboLeg[], minimumTotalOdds: number) => {
  ...
  // Lower is better: reward strong evidence, lightly penalize riskier lanes,
  // and prefer the smallest total SP that still clears the requested floor.
  return {
    totalOdds,
    averageEvidenceScore,
    score: distanceAboveFloor * 22
      + hhadCount * 4
      + highOddsCount * 3
      - averageEvidenceScore / 10,
  };
};

const chooseBestCombination = (
  candidates: FeaturedComboLeg[],
  size: FeaturedComboSize,
): FeaturedCombo => {
  ...
  const walk = (start: number, picked: FeaturedComboLeg[]) => {
    if (picked.length === size) {
      ...
      if (
        ...
        || (Math.abs(rank.score - bestRank.score) < 1e-9
          && rank.averageEvidenceScore > bestRank.averageEvidenceScore)
      ) {
        best = [...picked];
        bestRank = rank;
      }
    ...
    for (let index = start; index < candidates.length; index += 1) {
      walk(index + 1, [...picked, candidates[index]]);
    }
  };
  ...
  if (!best || !bestRank) {
    return {
      ...
      legs: [],
      status: 'insufficient',
      reason: `No ${size}-match combination clears the quality and SP floor.`,
    };
  }
  ...
  return {
    ...
    averageEvidenceScore: Number(bestRank.averageEvidenceScore.toFixed(1)),
    legs: best,
    status: 'ready',
  };
};
...
export const buildDailyFeaturedCombos = (matches: Match[]) => {
  ...
  return {
    candidates,
    two: chooseBestCombination(candidates, 2),
    three: chooseBestCombination(candidates, 3),
  };
};
