...
RUNTIME_FILES=(package.json package-lock.json cli.cjs scheduled.cjs scope.cjs store.cjs browser.cjs mapping.cjs runtime-policy.cjs failure-policy.cjs website-adapter.cjs website-reader.cjs schema.sql local-browser-ingest.cjs local-browser-schema.sql local-jingcai-day.cjs local-jingcai-scope.cjs local-jingcai-server.cjs)
